...
# Panel Bridge
try:
    from panel_bridge import PanelBridge as _PanelBridge
    _PANEL_BRIDGE_OK = True
except ImportError:
    print("[WARN] panel_bridge.py bulunamadı, panel entegrasyonu devre dışı.")
    _PANEL_BRIDGE_OK = False
...
# key -> {"name": str, "sheet_id": str, "tab": str, "emoji": str, "type": str}
# type: "content" veya "mass"
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            for k, v in data.items():
                ...
                stype = str(v.get("type", "content")).strip().lower() or "content"
                if stype not in ("content", "mass"):
                    stype = "content"
                if name and sheet_id and tab:
                    out[k.strip()] = {"name": name, "sheet_id": sheet_id, "tab": tab, "emoji": emoji, "type": stype}
            DYNAMIC_SHEETS = out
        else:
            DYNAMIC_SHEETS = {}
    except Exception:
...
DEFAULT_MOUNT_MASS = "Swiftclaw / Seripençe"  # Mass content için default mount
...
# =========================================================
ACTIVITY_STATE_FILE = os.getenv("ACTIVITY_STATE_FILE", os.path.join(BASE_DIR, "activity_state.json"))
ACTIVITY_INACTIVITY_DAYS = 5  # Kaç gün sonra DM gönderilsin


# =========================================================
#           PUAN SİSTEMİ (Voice Tabanlı - Modüler)
# =========================================================
"""
Puan Sistemi Kuralları:
- Voice verisi ayrı Voice Tracker Bot tarafından activity_state.json'a yazılır
- Bu bot sadece voice verisini OKUR
- 1 puan = 2 dakika (0.5 puan/dakika)
- Günlük maksimum: 60 puan (120 dakika puan kazandırır)
- Her gün TR 00:00'da günlük limitler sıfırlanır
- Toplam puan ≤ -3000 → UYARI
- Toplam puan ≤ -5000 → KICK (otomatik değil, sadece log/embed)
"""

PUAN_STATE_FILE = os.path.join(BASE_DIR, os.getenv("PUAN_STATE_FILE", "puan_state.json"))

# Puan Sabitleri
PUAN_PER_MINUTE = 0.5           # Dakika başına puan
PUAN_DAILY_MAX = 60.0           # Günlük maksimum puan
PUAN_DAILY_MAX_MINUTES = 120    # 60 puan = 120 dakika
PUAN_WARNING_THRESHOLD = -3000  # Uyarı eşiği
PUAN_KICK_THRESHOLD = -5000     # Kick eşiği (otomatik değil)

# Puan Log Kanalı (opsiyonel)
PUAN_LOG_CHANNEL_ID = int(os.getenv("PUAN_LOG_CHANNEL_ID", "0"))

def _load_puan_state() -> Dict[str, Any]:
    """Puan state dosyasını yükler."""
    try:
        if os.path.exists(PUAN_STATE_FILE):
            with open(PUAN_STATE_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                # Yapı doğrulama
        ...
        print(f"[PUAN] State yükleme hatası: {e}")
    return {"users": {}, "last_reset_date": None, "warned_users": [], "kick_warned_users": []}

def _save_puan_state(state: Dict[str, Any]) -> None:
    """Puan state dosyasını kaydeder."""
    try:
        with open(PUAN_STATE_FILE, "w", encoding="utf-8") as f:
            json.dump(state, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[PUAN] State kaydetme hatası: {e}")

def _get_today_tr() -> str:
    """Bugünün tarihini TR timezone'da döndürür (YYYY-MM-DD)."""
    now_tr = datetime.now(TR_TZ)
    return now_tr.strftime("%Y-%m-%d")

def _get_user_puan_data(state: Dict[str, Any], user_id: int) -> Dict[str, Any]:
    """Kullanıcının puan verisini döndürür, yoksa oluşturur."""
    user_id_str = str(user_id)
    if user_id_str not in state["users"]:
        state["users"][user_id_str] = {
            "total_points": 0.0,          # Toplam puan (tüm zamanlar)
            "daily_points": 0.0,          # Bugün kazanılan puan
            "daily_minutes_counted": 0,   # Bugün sayılan dakika
            "last_voice_minutes": 0,      # Son okunan voice_minutes değeri
            "last_update_date": None,     # Son güncelleme tarihi
        }
    return state["users"][user_id_str]

def _check_and_reset_daily(state: Dict[str, Any]) -> bool:
    """
    TR 00:00 reset kontrolü yapar.
    Eğer yeni gün başladıysa tüm günlük sayaçları sıfırlar.
    Returns: True if reset yapıldı
    """
    today = _get_today_tr()
    last_reset = state.get("last_reset_date")
    
    if last_reset != today:
        print(f"[PUAN] Günlük reset yapılıyor: {last_reset} -> {today}")
        
        for user_id_str, user_data in state.get("users", {}).items():
            user_data["daily_points"] = 0.0
            user_data["daily_minutes_counted"] = 0
            # last_voice_minutes'ı SIFIRLAMIYORUZ - fark hesabı için gerekli
        
        state["last_reset_date"] = today
        return True
    return False

def _calculate_voice_points(user_id: int, current_voice_minutes: int, puan_state: Dict[str, Any]) -> Tuple[float, int]:
    """
    Voice dakikalarından puan hesaplar.
    
    Returns: (kazanılan_puan, sayılan_dakika)
    
    Kurallar:
    - Sadece yeni dakikalar sayılır (current - last)
    - Günlük max 60 puan (120 dakika)
    - Günlük limit aşılırsa puan yazılmaz ama dakika sayılmaya devam eder
    """
    user_data = _get_user_puan_data(puan_state, user_id)
    
    last_minutes = user_data.get("last_voice_minutes", 0) or 0
    new_minutes = max(0, current_voice_minutes - last_minutes)
    
    if new_minutes <= 0:
        return 0.0, 0
    
    # Günlük limit kontrolü
    daily_points = user_data.get("daily_points", 0.0) or 0.0
    daily_minutes = user_data.get("daily_minutes_counted", 0) or 0
    
    # Kalan puan kapasitesi
    remaining_daily_points = max(0, PUAN_DAILY_MAX - daily_points)
    remaining_daily_minutes = int(remaining_daily_points / PUAN_PER_MINUTE)
    
    # Sayılacak dakika (günlük limiti aşmayacak şekilde)
    countable_minutes = min(new_minutes, remaining_daily_minutes)
    earned_points = countable_minutes * PUAN_PER_MINUTE
    
    return earned_points, new_minutes  # new_minutes: toplam yeni dakika (limit dahil değil)

def _update_user_points_from_voice(user_id: int, current_voice_minutes: int) -> Dict[str, Any]:
    """
    Kullanıcının puanını voice dakikalarından günceller.
    ...
    """
    puan_state = _load_puan_state()
    
    # Günlük reset kontrolü
    _check_and_reset_daily(puan_state)
    
    user_data = _get_user_puan_data(puan_state, user_id)
    today = _get_today_tr()
    
    earned_points, new_minutes = _calculate_voice_points(user_id, current_voice_minutes, puan_state)
    
    # Günlük limit kontrolü
    daily_points_before = user_data.get("daily_points", 0.0) or 0.0
    limit_reached = (daily_points_before + earned_points) >= PUAN_DAILY_MAX
    
    # Güncelle
    user_data["total_points"] = (user_data.get("total_points", 0.0) or 0.0) + earned_points
    user_data["daily_points"] = min(PUAN_DAILY_MAX, daily_points_before + earned_points)
    user_data["daily_minutes_counted"] = (user_data.get("daily_minutes_counted", 0) or 0) + new_minutes
    user_data["last_voice_minutes"] = current_voice_minutes
    user_data["last_update_date"] = today
    
    _save_puan_state(puan_state)
    
    return {
        "earned_points": earned_points,
        "total_points": user_data["total_points"],
        "daily_points": user_data["daily_points"],
        "daily_minutes": user_data["daily_minutes_counted"],
        "new_minutes": new_minutes,
        "limit_reached": limit_reached
    }

def _get_user_total_points(user_id: int) -> float:
    """Kullanıcının toplam puanını döndürür."""
    puan_state = _load_puan_state()
    user_data = _get_user_puan_data(puan_state, user_id)
    return user_data.get("total_points", 0.0) or 0.0

def _set_user_total_points(user_id: int, points: float) -> None:
    """Kullanıcının toplam puanını manuel ayarlar."""
    puan_state = _load_puan_state()
    user_data = _get_user_puan_data(puan_state, user_id)
    user_data["total_points"] = points
    _save_puan_state(puan_state)

def _check_puan_thresholds(user_id: int) -> Dict[str, Any]:
    """
    Puan eşiklerini kontrol eder.
    
    Returns: {
        "total_points": float,
        "needs_warning": bool,  # -3000 altı
        "needs_kick_warning": bool,  # -5000 altı
        "already_warned": bool,
        "already_kick_warned": bool
    }
    """
    puan_state = _load_puan_state()
    user_data = _get_user_puan_data(puan_state, user_id)
    total_points = user_data.get("total_points", 0.0) or 0.0
    
    user_id_str = str(user_id)
    warned_users = puan_state.get("warned_users", [])
    kick_warned_users = puan_state.get("kick_warned_users", [])
    
    needs_warning = total_points <= PUAN_WARNING_THRESHOLD
    needs_kick_warning = total_points <= PUAN_KICK_THRESHOLD
    already_warned = user_id_str in warned_users
    already_kick_warned = user_id_str in kick_warned_users
    
    return {
        "total_points": total_points,
        "needs_warning": needs_warning,
        "needs_kick_warning": needs_kick_warning,
        "already_warned": already_warned,
        "already_kick_warned": already_kick_warned
    }

def _mark_user_warned(user_id: int, warn_type: str = "warning") -> None:
    """Kullanıcıyı uyarıldı olarak işaretler."""
    puan_state = _load_puan_state()
    user_id_str = str(user_id)
    
    if warn_type == "kick":
        if "kick_warned_users" not in puan_state:
            puan_state["kick_warned_users"] = []
        if user_id_str not in puan_state["kick_warned_users"]:
            puan_state["kick_warned_users"].append(user_id_str)
    else:
        if "warned_users" not in puan_state:
            puan_state["warned_users"] = []
        if user_id_str not in puan_state["warned_users"]:
            puan_state["warned_users"].append(user_id_str)
    
    _save_puan_state(puan_state)

def _clear_user_warnings(user_id: int) -> None:
    """Kullanıcının uyarılarını temizler (puan yükseldiğinde)."""
    puan_state = _load_puan_state()
    user_id_str = str(user_id)
    
    if user_id_str in puan_state.get("warned_users", []):
        puan_state["warned_users"].remove(user_id_str)
    if user_id_str in puan_state.get("kick_warned_users", []):
        puan_state["kick_warned_users"].remove(user_id_str)
    
    _save_puan_state(puan_state)


# =========================================================
#           CONTENT LOG SİSTEMİ (Google Sheets)
# =========================================================
"""
Content Log Yapısı (YENİ):
- A sütunu: Member listesi (A4'ten başlar)
- A3: "Content" header
- Her content 2 sütun kullanır: B:C, D:E, F:G...
  - B1:C1 = Content adı (merge edilmiş)
  - B2 = Tarih, C2 = Saat
  - B3 = "Loot", C3 = "✅" 
  - B4+ = Loot miktarı (varsa), C4+ = ✅ (katıldıysa)

Örnek:
|    A (Üyeler)   |   B:C (AVALON)  |  D:E (STATIC)  |
|-----------------|-----------------|----------------|
| (boş)           |     AVALON      |    STATIC      | <- merge
| (boş)           | 2026-01-28|21:00| 2026-01-28|19:30|
| Content         |   Loot   | ✅   |   Loot  | ✅   |
| Tunç            | 847.000  | ✅   |         | ✅   |
"""

CONTENT_LOG_TAB_NAME = os.getenv("CONTENT_LOG_TAB_NAME", "Content Log").strip() or "Content Log"
CONTENT_LOG_MEMBER_START_ROW = 4  # A4'ten başlar

def _get_content_log_worksheet():
    """Content Log worksheet'ini döndürür."""
    sheet_id = ACTIVITY_SHEET_ID
    if not sheet_id:
        raise RuntimeError("ACTIVITY_SHEET_ID ayarlı değil.")
    
    sh = _gs_open_sheet_sync(sheet_id)
    try:
        return sh.worksheet(CONTENT_LOG_TAB_NAME)
    except Exception:
        ws = sh.add_worksheet(title=CONTENT_LOG_TAB_NAME, rows=500, cols=100)
        ws.update_acell("A3", "Content")
        return ws

def _get_sheets_service():
    """Google Sheets API service döndürür (merge işlemleri için)."""
...
def _merge_cells(sheet_id: int, start_row: int, end_row: int, start_col: int, end_col: int):
    """Google Sheets API ile hücreleri birleştirir."""
    try:
        ...
        service.spreadsheets().batchUpdate(
            spreadsheetId=ACTIVITY_SHEET_ID,
            body=request
        ).execute()
        return True
    except Exception as e:
        log(f"[SHEETS] Merge hatası: {e}")
        return False

def _get_worksheet_id(ws) -> int:
    """Worksheet'in ID'sini döndürür."""
    return ws.id

async def _sync_members_to_content_log(bot_client) -> int:
    """Member rolü olan tüm üyeleri Content Log'a yazar."""
    def _do_sync():
        guild = bot_client.get_guild(GUILD_ID)
        if not guild:
            return 0
        
        member_role = guild.get_role(ACTIVITY_MEMBER_ROLE_ID)
        if not member_role:
            return 0
        
        ws = _get_content_log_worksheet()
        
        members_data = []
        for member in guild.members:
            if member.bot or member_role not in member.roles:
                continue
            members_data.append(member.display_name)
        
        if not members_data:
            return 0
        
        members_data.sort(key=lambda x: x.lower())
        
        try:
            ws.batch_clear([f"A{CONTENT_LOG_MEMBER_START_ROW}:A500"])
        except Exception:
            pass
        
        start_row = CONTENT_LOG_MEMBER_START_ROW
        end_row = start_row + len(members_data) - 1
        ws.update(
            values=[[name] for name in members_data],
        ...
        log(f"[CONTENT LOG] {len(members_data)} üye yazıldı.")
        return len(members_data)
    
    return await run_io(_do_sync)

async def _add_content_to_log(content_name: str, content_date: str, content_time: str) -> Tuple[int, int]:
    """
    Yeni content ekler (2 sütun, merge ile).
    Returns: (loot_col, tick_col) tuple
    """
    def _do_add():
        ws = _get_content_log_worksheet()
        sheet_id = _get_worksheet_id(ws)
        content_lower = content_name.strip().lower()
        
        row1 = ws.row_values(1)
        
        # Mevcut content'i ara (2'li atlayarak)
        col_idx = 1  # B sütunu (0-indexed)
        while col_idx < len(row1):
            val = (row1[col_idx] or "").strip().lower()
            if val == content_lower:
                loot_col = col_idx + 1
                tick_col = col_idx + 2
                return (loot_col, tick_col)
            col_idx += 2
        
        # Yeni sütun bul
        next_col = 2  # B (1-indexed)
        col_idx = 1
        while col_idx < len(row1):
            if not (row1[col_idx] or "").strip():
                next_col = col_idx + 1
                break
            col_idx += 2
        else:
        ...
        loot_letter = _col_num_to_letter(loot_col)
        tick_letter = _col_num_to_letter(tick_col)
        
        # Merge header (row 1)
        _merge_cells(sheet_id, 0, 1, loot_col - 1, tick_col)
        
        # Content adını yaz
        ws.update_acell(f"{loot_letter}1", content_name)
        
        # Tarih ve saat
        ws.update_acell(f"{loot_letter}2", content_date)
        ws.update_acell(f"{tick_letter}2", content_time)
        
        # Sub-headers
        ws.update_acell(f"{loot_letter}3", "Loot")
        ws.update_acell(f"{tick_letter}3", "✅")
        
        log(f"[CONTENT LOG] Content eklendi: {content_name} @ {loot_letter}:{tick_letter}")
        return (loot_col, tick_col)
    
    return await run_io(_do_add)

async def _mark_content_participation(tick_col: int, participant_names: List[str]) -> int:
    """Katılımcıları ✅ ile işaretler."""
    def _do_mark():
        ws = _get_content_log_worksheet()
        col_a = ws.col_values(1)
        
        marked = 0
        col_letter = _col_num_to_letter(tick_col)
        updates = []
        
        for row_idx, name in enumerate(col_a, start=1):
            if row_idx < CONTENT_LOG_MEMBER_START_ROW:
                continue
            
            name_lower = (name or "").strip().lower()
            for participant in participant_names:
                if participant.lower() == name_lower:
                    updates.append({
        ...
        log(f"[CONTENT LOG] {marked} katılımcı işaretlendi @ {col_letter}")
        return marked
    
    return await run_io(_do_mark)

def _col_num_to_letter(col: int) -> str:
    """Sütun numarasını harf(ler)e çevirir. 1=A, 2=B, 27=AA, vb."""
    result = ""
    while col > 0:
...
# =========================================================
#           PUAN LOG SİSTEMİ (Google Sheets - Günlük Takip)
# =========================================================
"""
Puan Log Yapısı (YENİ):
- A sütunu: Member Name (A2'den başlar, A1="Üye")
- B sütunu: IGN (B1="IGN")
- C sütunu: Toplam Puan (formül)
- D'den itibaren: Her gün için 2 sütun
  - D1:E1 = Tarih (merge edilmiş, "28.01" formatı)
  - D2+ = Voice dakika (o gün)
  - E2+ = Content sayısı (o gün kaç content'e katıldı)

Örnek:
| Üye  | IGN    | Toplam | 28.01      | 29.01      |
|------|--------|--------|------------|------------|
|      |        |        | V    | C   | V    | C   |  <- (sub-header yok, direkt veri)
| Tunç | TuncAO | 195    | 45   | 1   | 60   | 2   |

Puan Hesaplama (Activity Balance'da):
- Voice: 0.5 puan/dakika, max 60/gün
- Content: +150 katıldı, -250 katılmadı (zorunlu), 0 opsiyonel
"""

PUAN_LOG_TAB_NAME = os.getenv("PUAN_LOG_TAB_NAME", "Puan Log").strip() or "Puan Log"
PUAN_LOG_MEMBER_START_ROW = 2  # A2'den başlar

# Content puan sabitleri
CONTENT_JOINED_POINTS = 150
CONTENT_MISSED_POINTS = -250
CONTENT_NONE_POINTS = 0

def _get_puan_log_worksheet():
    """Puan Log worksheet'ini döndürür."""
    import gspread
    sheet_id = ACTIVITY_SHEET_ID
    if not sheet_id:
        raise RuntimeError("ACTIVITY_SHEET_ID ayarlı değil.")
    ...
    sh = _gs_open_sheet_sync(sheet_id)
    try:
        return sh.worksheet(PUAN_LOG_TAB_NAME)
    except gspread.exceptions.WorksheetNotFound:
        import time
        time.sleep(1)
        ws = sh.add_worksheet(title=PUAN_LOG_TAB_NAME, rows=500, cols=200)
        time.sleep(1)
        ws.update_acell("A1", "Üye")
        ws.update_acell("B1", "IGN")
        ws.update_acell("C1", "Toplam")
        return ws

def _get_today_date_str() -> str:
    """Bugünün tarihini DD.MM formatında döndürür."""
    return datetime.now(TR_TZ).strftime("%d.%m")

def _find_or_create_daily_columns(ws, date_str: str) -> Tuple[int, int]:
    """
    Belirtilen tarih için Voice ve Content sütunlarını bulur veya oluşturur.
    Tarih header'ı merge edilir.
    Returns: (voice_col_num, content_col_num)
    """
    row1 = ws.row_values(1)
    
    # Mevcut sütunlarda ara (2'li atlayarak, D'den başla)
    col_idx = 3  # D sütunu (0-indexed)
    while col_idx < len(row1):
        val = (row1[col_idx] or "").strip()
        if val == date_str:
            voice_col = col_idx + 1
            content_col = col_idx + 2
            return (voice_col, content_col)
        col_idx += 2
    
    # Bulunamadı, yeni sütunlar oluştur
    next_col = 4  # D sütunu (1-indexed)
    col_idx = 3
    while col_idx < len(row1):
        if not (row1[col_idx] or "").strip():
            next_col = col_idx + 1
        ...
        next_col = len(row1) + 1
        # Çift sütunda başla (D=4, F=6, H=8...)
        if (next_col - 4) % 2 == 1:
            next_col += 1
    
    voice_col = next_col
    content_col = next_col + 1
    
    voice_letter = _col_num_to_letter(voice_col)
    content_letter = _col_num_to_letter(content_col)
    
    # Merge header
    sheet_id = _get_worksheet_id(ws)
    _merge_cells(sheet_id, 0, 1, voice_col - 1, content_col)
    
    # Tarih yaz
    ws.update_acell(f"{voice_letter}1", date_str)
    
    log(f"[PUAN LOG] Günlük sütunlar: {date_str} @ {voice_letter}:{content_letter}")
    return (voice_col, content_col)

def _find_member_row_puan(ws, member_name: str, ign: str, col_a: List = None, col_b: List = None) -> Optional[int]:
    """Üyenin satır numarasını bulur. col_a/col_b cache olarak verilebilir."""
    if col_a is None:
        col_a = ws.col_values(1)
    if col_b is None:
        try:
    ...
    ign_lower = (ign or "").strip().lower()
    
    for row_idx, val in enumerate(col_a):
        if row_idx < PUAN_LOG_MEMBER_START_ROW - 1:
            continue
        
        name_lower = (val or "").strip().lower()
        
        if member_lower in name_lower or name_lower in member_lower:
            return row_idx + 1
        
        if ign_lower and row_idx < len(col_b):
            ign_val = (col_b[row_idx] or "").strip().lower()
            if ign_lower in ign_val or ign_val in ign_lower:
                return row_idx + 1
    
    return None

def _add_member_to_puan_log_batch(ws, member_name: str, ign: str, col_a: List) -> int:
    """Yeni üye satır numarasını hesaplar (batch için). Yazmaz!"""
    next_row = PUAN_LOG_MEMBER_START_ROW
    for i, val in enumerate(col_a):
        if i < PUAN_LOG_MEMBER_START_ROW - 1:
            continue
...
def _add_member_to_puan_log(ws, member_name: str, ign: str) -> int:
    """Yeni üye ekler, satır numarası döndürür."""
    col_a = ws.col_values(1)
    ...
    ws.update_acell(f"A{next_row}", member_name)
    if ign:
        ws.update_acell(f"B{next_row}", ign)
    
    ws.update_acell(f"C{next_row}", f"=SUM(D{next_row}:ZZ{next_row})")
    
    log(f"[PUAN LOG] Yeni üye: {member_name} @ satır {next_row}")
    return next_row

async def _sync_members_to_puan_log(bot_client) -> int:
    """Member rolü olan tüm üyeleri Puan Log'a ekler. (Optimized - batch)"""
    def _do_sync():
        import time
        ...
        ws = _get_puan_log_worksheet()
        
        # Tüm verileri BİR KERE oku (rate limit için kritik!)
        col_a = ws.col_values(1)
        try:
            col_b = ws.col_values(2)
        except Exception:
        ...
        for member in guild.members:
            ...
            ign = ""
            if member.id in _player_links:
                ign = _player_links[member.id].albion_name
            
            # Cache'li arama
            row = _find_member_row_puan(ws, member_name, ign, col_a, col_b)
            if row is None:
                new_members.append((member_name, ign))
        
        if not new_members:
            log("[PUAN LOG] Yeni üye yok.")
            return 0
        ...
        updates = []
        for idx, (name, ign) in enumerate(new_members):
            row = next_row + idx
            updates.append({"range": f"A{row}", "values": [[name]]})
            if ign:
                updates.append({"range": f"B{row}", "values": [[ign]]})
            updates.append({"range": f"C{row}", "values": [[f"=SUM(D{row}:ZZ{row})"]]})
        
        if updates:
            # Batch update (tek API çağrısı)
            ws.batch_update(updates, value_input_option='USER_ENTERED')
            time.sleep(1)  # Rate limit koruması
        
        log(f"[PUAN LOG] {len(new_members)} yeni üye eklendi (batch).")
        return len(new_members)
    
    return await run_io(_do_sync)

async def _write_daily_voice_to_puan_log(bot_client) -> int:
    """
    Bugünün voice DAKİKALARINI Puan Log'a yazar.
    Voice Tracker'dan activity_state.json'dan GÜNLÜK dakikayı okur.
    (Optimized - tek okuma, batch yazma)
    """
    def _do_write():
        import time
        ...
        voice_col, _ = _find_or_create_daily_columns(ws, today)
        voice_letter = _col_num_to_letter(voice_col)
        
        # Tüm verileri BİR KERE oku
        col_a = ws.col_values(1)
        try:
            col_b = ws.col_values(2)
        except Exception:
        ...
        for member in guild.members:
            ...
            row = _find_member_row_puan(ws, member_name, ign, col_a, col_b)
            if row is None:
                # Yeni üye - listeye ekle, sonra batch yazacağız
                new_members.append((member_name, ign, member.id))
                continue
            
            # Günlük voice dakikasını al
            user_puan = puan_state.get("users", {}).get(str(member.id), {})
            daily_minutes = user_puan.get("daily_minutes_counted", 0) or 0
            
            if daily_minutes > 0:
                updates.append({
                    "range": f"{voice_letter}{row}",
                    "values": [[daily_minutes]]
                })
                count += 1
        
        # Yeni üyeleri önce ekle
        if new_members:
            next_row = _add_member_to_puan_log_batch(ws, "", "", col_a)
            member_updates = []
            for idx, (name, ign, uid) in enumerate(new_members):
                row = next_row + idx
                member_updates.append({"range": f"A{row}", "values": [[name]]})
                if ign:
                    member_updates.append({"range": f"B{row}", "values": [[ign]]})
                member_updates.append({"range": f"C{row}", "values": [[f"=SUM(D{row}:ZZ{row})"]]})
                
                # Voice dakikasını da ekle
                user_puan = puan_state.get("users", {}).get(str(uid), {})
                daily_minutes = user_puan.get("daily_minutes_counted", 0) or 0
                if daily_minutes > 0:
                    updates.append({"range": f"{voice_letter}{row}", "values": [[daily_minutes]]})
                    count += 1
            
            if member_updates:
                ws.batch_update(member_updates, value_input_option='USER_ENTERED')
                time.sleep(1)
        
        if updates:
            ws.batch_update(updates)
        
        log(f"[PUAN LOG] {count} üyenin voice dakikası yazıldı ({today})")
        return count
    
    return await run_io(_do_write)

async def _write_content_count_to_puan_log(participant_names: List[str]) -> int:
    """
    Bugün bir content oldu - katılanların content sayısını +1 artırır.
    """
    def _do_write():
        ws = _get_puan_log_worksheet()
        today = _get_today_date_str()
        
        _, content_col = _find_or_create_daily_columns(ws, today)
        content_letter = _col_num_to_letter(content_col)
        
        # Mevcut content sayılarını oku
        try:
            content_values = ws.col_values(content_col)
        except Exception:
            content_values = []
        ...
        for row_idx, name in enumerate(col_a, start=1):
            if row_idx < PUAN_LOG_MEMBER_START_ROW:
                continue
            ...
            for participant in participant_names:
                if participant.lower() == name_lower:
                    # Mevcut değeri al ve +1 ekle
                    current = 0
                    if row_idx <= len(content_values):
                        try:
                            current = int(content_values[row_idx - 1] or 0)
                        except (ValueError, TypeError):
                            current = 0
                    
                    updates.append({
                        "range": f"{content_letter}{row_idx}",
                        "values": [[current + 1]]
                    })
                    count += 1
                    break
        ...
        log(f"[PUAN LOG] {count} üyenin content sayısı güncellendi ({today})")
        return count
...
def _get_user_activity(state: Dict[str, Any], user_id: int) -> Dict[str, Any]:
    ...
    if user_id_str not in state["users"]:
        state["users"][user_id_str] = {
            "content_joins": 0,
            "voice_minutes": 0,  # Voice bot tarafından yazılır, ana bot sadece okur
            "last_activity": None,
...
def _update_activity(user_id: int, activity_type: str, **kwargs):
    """Kullanıcının aktivitesini günceller. (Sadece content takibi - voice ayrı bot tarafından yapılır)"""
...
# ── Fallback PRESETS (JSON yüklenemezse kullanılır) ─────────────────────────
_FALLBACK_PRESETS: Dict[str, EventTemplate] = {
    ...
    "statik": EventTemplate("statik", "🇸 🇹 🇦 🇹 ℹ️ 🇨", "", "STATIK",
                            [("tank", 1), ("healer", 1), ("sc", 1), ("perma", 1), ("wailing", 3), ("fill", 999)]),
    "static_speed": EventTemplate("static_speed", "STATIC SPEED", "", "STATIC SPEED",
...
# ── content_templates.json'dan PRESETS yükle ────────────────────────────────
_CONTENT_TEMPLATES_FILE = os.path.join(BASE_DIR, "data", "content_templates.json")
# Emoji → kategori map (ContentSelect/MassSelect için)
_TEMPLATE_EMOJIS: Dict[str, str] = {}   # key -> emoji
_TEMPLATE_CATEGORY: Dict[str, str] = {} # key -> "content" | "mass"
_TEMPLATE_ORDER: List[str] = []         # ordered key list

def _load_presets_from_json() -> Dict[str, EventTemplate]:
    """
    data/content_templates.json'dan EventTemplate dict'i oluşturur.
    Hata durumunda fallback PRESETS'e döner.
    """
    global _TEMPLATE_EMOJIS, _TEMPLATE_CATEGORY, _TEMPLATE_ORDER
    try:
        if not os.path.exists(_CONTENT_TEMPLATES_FILE):
            log(f"[TEMPLATES] {_CONTENT_TEMPLATES_FILE} bulunamadı, fallback kullanılıyor.")
            return dict(_FALLBACK_PRESETS)

        with open(_CONTENT_TEMPLATES_FILE, "r", encoding="utf-8") as f:
            raw = json.load(f)

        if not isinstance(raw, list) or not raw:
            log("[TEMPLATES] content_templates.json boş veya liste değil, fallback kullanılıyor.")
            return dict(_FALLBACK_PRESETS)

        result: Dict[str, EventTemplate] = {}
        emojis: Dict[str, str] = {}
        cats: Dict[str, str] = {}
        order: List[str] = []

        for item in sorted(raw, key=lambda x: x.get("order", 999)):
            try:
                ...
                thread_name = str(item.get("thread_name", key.upper()))
                ...
                roles: List[Tuple[str, int]] = []
                for r in raw_roles:
                    rname = str(r.get("role_name", "")).strip()
                    rcap = int(r.get("capacity", 1))
                    if rname:
                        roles.append((rname, rcap))

                if not roles:
                    log(f"[TEMPLATES] '{key}' için rol yok, atlandı.")
                    continue

                result[key] = EventTemplate(key, title, subtitle, thread_name, roles)
                emojis[key] = emoji
                cats[key] = category
                order.append(key)
            except Exception as e:
                log(f"[TEMPLATES] Template parse hatası: {e}")
                continue

        if not result:
            log("[TEMPLATES] Hiç template yüklenemedi, fallback kullanılıyor.")
            return dict(_FALLBACK_PRESETS)

        _TEMPLATE_EMOJIS = emojis
        _TEMPLATE_CATEGORY = cats
        _TEMPLATE_ORDER = order
        log(f"[TEMPLATES] {len(result)} template yüklendi: {', '.join(order)}")
        return result

    except Exception as e:
        log(f"[TEMPLATES] Yükleme hatası: {e}, fallback kullanılıyor.")
        return dict(_FALLBACK_PRESETS)

def reload_presets() -> None:
    """PRESETS'i yeniden yükler (bot çalışırken JSON değişince)."""
    global PRESETS
    PRESETS = _load_presets_from_json()
    log(f"[TEMPLATES] PRESETS yeniden yüklendi: {len(PRESETS)} template.")

PRESETS: Dict[str, EventTemplate] = _load_presets_from_json()
...
async def gs_append_row(tab_name: str, values: List[str]) -> None:
    """Append a single row to a worksheet. Creates the worksheet if missing (best-effort)."""
    def _do():
        sid, tab = _split_sheet_ref(tab_name)
        sheet_id = sid or _resolve_sheet_id_for_tab(tab)
        if not sheet_id:
        ...
        # Determine next row index (best-effort) BEFORE append
        try:
        ...
        # Optional: grey background for Content Log rows (artık kullanılmıyor)
        # Eski tek satırlık log sistemi kaldırıldı
...
def _build_variant_label(headers: List[str], rows: List['SheetRoleRow'], sig_groups: Dict[str, List['SheetRoleRow']]) -> Dict[str, str]:
    """Build a human-friendly suffix for duplicated roles.

    Instead of (1)/(2), we try to display the differentiating item (e.g., Helmet/Armor).
    Returned values are suffixes WITHOUT parentheses. Caller wraps them like: f"{role} ({suffix})".
    """
    if len(sig_groups) <= 1:
        only_sig = next(iter(sig_groups.keys()))
        return {only_sig: ""}

    # Representative row per signature
    rep: Dict[str, 'SheetRoleRow'] = {sig: (sub[0] if sub else None) for sig, sub in sig_groups.items()}

    # Helper: find a header name by fuzzy matching
    def _pick_header(candidates: List[str]) -> Optional[str]:
        hs = [(h or "").strip() for h in headers]
        lowers = [h.lower() for h in hs]
        for cand in candidates:
            c = cand.lower()
            # exact
            if c in lowers:
                return hs[lowers.index(c)]
            # contains
            for i, lh in enumerate(lowers):
                if c in lh:
                    return hs[i]
        return None

    # Priority: choose a column that actually varies across variants
    priority_groups = [
        ["helmet", "head"],
        ["armor", "chest"],
        ["shoes", "boots"],
    ...
    for cand_group in priority_groups:
        hname = _pick_header(cand_group)
        if not hname:
            continue
        vals = []
        for sig, rr in rep.items():
            if rr is None:
                continue
            v = (rr.values.get(hname) or "").strip()
            vals.append(v)
        # must vary and must have at least 2 non-empty different values
        uniq = {v for v in vals if v}
        if len(uniq) >= 2:
            chosen_header = hname
            break

    # Fallback to numbering if we couldn't find a varying column
    ...
    # Build suffix by the chosen header value; ensure uniqueness, otherwise add 2nd header
    suffixes: Dict[str, str] = {}
    for sig, rr in rep.items():
        v = (rr.values.get(chosen_header) or "").strip() if rr else ""
        suffixes[sig] = v or "Alt Build"

    # If still not unique, append another varying column (armor/shoes) to disambiguate
    if len(set(suffixes.values())) < len(suffixes):
        # try to find a second header that varies and isn't the chosen one
        secondary = None
        for cand_group in priority_groups:
            h2 = _pick_header(cand_group)
            if not h2 or h2 == chosen_header:
                continue
            vals2 = []
            for sig, rr in rep.items():
                if rr is None:
                    continue
                vals2.append((rr.values.get(h2) or "").strip())
            uniq2 = {v for v in vals2 if v}
            if len(uniq2) >= 2:
                secondary = h2
                break
        if secondary:
            for sig, rr in rep.items():
                v1 = suffixes.get(sig, "")
                v2 = (rr.values.get(secondary) or "").strip() if rr else ""
                if v2:
                    suffixes[sig] = f"{v1} / {v2}"
...
class SheetRoleEntry:
    ...
    total: int
    label: str
    emoji: str  # Sheet'ten Z sütunundan okunan emoji (örn: ":hallowfall:")
...
def build_role_entries(tab: str, st_slots: Dict[str, Tuple[str, Optional[int]]], headers: List[str], rows: List[SheetRoleRow]) -> List[SheetRoleEntry]:
    ...
    # Z sütunu emoji için (veya "Emoji" header'ı)
    emoji_col = None
    for i, h in enumerate(headers):
        h_lower = (h or "").strip().lower()
        if h_lower == "emoji" or h_lower == "z":
            emoji_col = h
            break
    # Z sütunu index 25 (0-based), header varsa onu kullan
    if emoji_col is None and len(headers) >= 26:
        emoji_col = headers[25]  # Z sütunu

    entries: List[SheetRoleEntry] = []
    for rk, rrlist in role_groups.items():
        role_name = rrlist[0].role
        
        # İlk row'dan emoji al (aynı role için aynı emoji varsayılır)
        role_emoji = ""
        if emoji_col:
            role_emoji = (rrlist[0].values.get(emoji_col) or "").strip()
...
class SheetEventState:
    ...
    def to_dict(self) -> dict:
        """JSON serialization için dict'e çevirir."""
        return {
            ...
            "thread_msg_id": self.thread_msg_id,
            "owner_id": self.owner_id,
            "toplanma": self.toplanma,
            "time_tr": self.time_tr,
            "time_utc": self.time_utc,
            "mount": self.mount,
            "ayar": self.ayar,
            # slots: {"variant_id": ("display_name", user_id|None)}
            "slots": {k: [v[0], v[1]] for k, v in self.slots.items()},
            # user_slot: {user_id: "variant_id"}
            "user_slot": {str(k): v for k, v in self.user_slot.items()},
            "page": self.page,
        }
    
    @classmethod
    def from_dict(cls, data: dict) -> "SheetEventState":
        """JSON'dan SheetEventState oluşturur."""
        return cls(
            ...
            thread_msg_id=data["thread_msg_id"],
            owner_id=data["owner_id"],
            toplanma=data["toplanma"],
            time_tr=data["time_tr"],
            time_utc=data["time_utc"],
            mount=data["mount"],
            ayar=data["ayar"],
            # slots: dict -> {str: tuple}
            slots={k: (v[0], v[1]) for k, v in data.get("slots", {}).items()},
            # user_slot: {str: str} -> {int: str}
            user_slot={int(k): v for k, v in data.get("user_slot", {}).items()},
            page=data.get("page", 0),
        )

SHEET_EVENTS: Dict[int, SheetEventState] = {}
SHEET_THREAD_TO_MAIN: Dict[int, int] = {}
SHEET_EVENTS_FILE = "sheet_events.json"

def _save_sheet_events():
    """Content event'lerini dosyaya kaydeder."""
    try:
        data = {str(k): v.to_dict() for k, v in SHEET_EVENTS.items()}
        with open(SHEET_EVENTS_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        log(f"[CONTENT] {len(SHEET_EVENTS)} event kaydedildi")
    except Exception as e:
        log(f"[CONTENT] Event kaydetme hatası: {e}")

def _load_sheet_events():
    """Content event'lerini dosyadan yükler."""
    global SHEET_EVENTS, SHEET_THREAD_TO_MAIN
    try:
        if os.path.exists(SHEET_EVENTS_FILE):
            with open(SHEET_EVENTS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            SHEET_EVENTS = {int(k): SheetEventState.from_dict(v) for k, v in data.items()}
            # Thread mapping'i de kur
            for msg_id, st in SHEET_EVENTS.items():
                SHEET_THREAD_TO_MAIN[st.thread_id] = msg_id
            log(f"[CONTENT] {len(SHEET_EVENTS)} event yüklendi")
    except Exception as e:
        log(f"[CONTENT] Event yükleme hatası: {e}")
        SHEET_EVENTS = {}

def _register_sheet_event_views(bot_client):
    """Kayıtlı content event'leri için view'ları yeniden bağlar."""
    count = 0
    for msg_id, st in SHEET_EVENTS.items():
        try:
            # Ana mesaj için view (SheetMainView)
            main_view = SheetMainView(st)
            bot_client.add_view(main_view, message_id=msg_id)
            
            # Thread mesajı için view (SheetThreadView) - varsa
            if st.thread_msg_id:
                # Thread view için page_entries ve total_pages lazım, basit view kullanalım
                # View'ın butonları persistent olduğu için sadece bağlamak yeterli
                pass  # Thread view'ı interaction'da yeniden oluşturulacak
            
            count += 1
        except Exception as e:
            log(f"[CONTENT] View kayıt hatası (msg={msg_id}): {e}")
    
    log(f"[CONTENT] {count} event view kaydedildi")
...
async def build_sheet_main_embed_async(st: SheetEventState, guild: discord.Guild) -> discord.Embed:
    ...
    try:
        ...
        # Tüm rolleri göster, 1024 karakteri geçince yeni field aç
        all_lines = []
        for ent in entries:
            emoji_prefix = f"{ent.emoji} " if getattr(ent, "emoji", None) else ""
            all_lines.append(f"• {emoji_prefix}**{ent.label}** - **{ent.filled}/{ent.total}**")
        
        # Field'lara böl (her biri max 1024 karakter)
        fields = []
        current_lines = []
        current_len = 0
        ...
        for line in all_lines:
            ...
            if current_len + line_len > 1000 and current_lines:
                fields.append("\n".join(current_lines))
                current_lines = []
                current_len = 0
        ...
        for idx, field_value in enumerate(fields):
            field_name = "Kadro" if idx == 0 else "─"  # İlk field "Kadro", sonrakiler sadece çizgi
            e.add_field(name=field_name, value=field_value if field_value else "-", inline=False)
        
        if not fields:
            e.add_field(name="Kadro", value="-", inline=False)
...
async def build_sheet_thread_embed(st: SheetEventState) -> Tuple[discord.Embed, List[SheetRoleEntry], int]:
    ...
    for i, ent in enumerate(page_entries, start=1):
        ...
        emoji_prefix = f"{ent.emoji} " if getattr(ent, "emoji", None) else ""
        all_lines.append(f"{emoji_prefix}{icon} **{ent.label}** - **{ent.filled}/{ent.total}**")
    ...
    # Field'ları ekle
    for idx, field_value in enumerate(fields):
        field_name = f"Seçenekler (Sayfa {page+1}/{total_pages})" if idx == 0 else "─"
        e.add_field(name=field_name, value=field_value if field_value else "-", inline=False)
    
    if not fields:
        e.add_field(name=f"Seçenekler (Sayfa {page+1}/{total_pages})", value="-", inline=False)
...
class SheetRoleSelect(discord.ui.Select):
    def __init__(self, st: SheetEventState, page_entries: List[SheetRoleEntry], page_index: int, total_pages: int):
        ...
        for i, ent in enumerate(page_entries, start=1):
            # Dropdown'da emoji göstermek için SelectOption.emoji kullanılmalı.
            opt_emoji = None
            try:
                s = (ent.emoji or "").strip()
                if s:
                    # Custom emoji: "<:NAME:ID>" veya "<a:NAME:ID>"
                    if s.startswith("<") and s.endswith(">"):
                        opt_emoji = discord.PartialEmoji.from_str(s)
                    else:
                        # Unicode emoji ise string olarak verilebilir
                        opt_emoji = s
            except Exception:
                opt_emoji = None

            label = f"{i}. {ent.label} - {ent.filled}/{ent.total}"
            opts.append(discord.SelectOption(label=label[:100], value=ent.variant_id, emoji=opt_emoji))
        super().__init__(
...
async def sheet_leave_user(bot_client: discord.Client, st: SheetEventState, user_id: int) -> Tuple[bool, str]:
    ...
    # Content event'i kaydet (bot restart için)
    _save_sheet_events()
...
class ContentSelect(discord.ui.Select):
    def __init__(self):
        ...
        # content_templates.json'dan "content" kategorisindekiler
        content_keys = [k for k in _TEMPLATE_ORDER if _TEMPLATE_CATEGORY.get(k, "content") == "content"]
        # Fallback: _TEMPLATE_ORDER boşsa PRESETS'ten al
        if not content_keys:
            content_keys = [k for k in PRESETS if _TEMPLATE_CATEGORY.get(k, "content") == "content"]

        max_opts = 25
        # dynamic sheet'ler için yer bırak (max 5) + Custom için 1
        preset_limit = max_opts - 6
        for k in content_keys[:preset_limit]:
            tpl = PRESETS.get(k)
            if not tpl:
                continue
            emoji = _TEMPLATE_EMOJIS.get(k, "📋")
            label = tpl.title[:100] if len(tpl.title) <= 25 else (tpl.thread_name or k)[:100]
            opts.append(discord.SelectOption(label=label[:100], value=k, emoji=emoji if len(emoji) <= 2 else None))

        # dynamic sheets - sadece "content" tipindekiler
        try:
            dyn_items = sorted(
                [(k, v) for k, v in DYNAMIC_SHEETS.items() if v.get("type", "content") == "content"],
        ...
        opts.append(discord.SelectOption(label="Custom", value="custom_manual", emoji="🛠️"))

        if not opts:
            opts.append(discord.SelectOption(label="Template bulunamadı", value="none"))

        super().__init__(placeholder="👇 Content seç", min_values=1, max_values=1, options=opts[:25])

    async def callback(self, interaction: discord.Interaction):
        key = (self.values[0] or "").strip()

        if key == "none":
            return await safe_send(interaction, "❌ Template bulunamadı.", ephemeral=True)
        ...
        # Eski sabit sheet anahtarları için geriye dönük uyumluluk
        if key in ("sheet_avaskip", "avaskip"):
            try:
                return await interaction.response.send_modal(SheetContentModal("avaskip"))
            except Exception as e:
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)
        if key in ("sheet_10man", "10man"):
            try:
                return await interaction.response.send_modal(SheetContentModal("10man"))
            except Exception as e:
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)
        if key in ("sheet_brawlcomp", "brawlcomp"):
            try:
                return await interaction.response.send_modal(SheetContentModal("brawlcomp"))
            except Exception as e:
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)

        # custom_all: başlık girilebilir modal
        if key == "custom_all" or key == "custom_manual":
            try:
        ...
        if key in PRESETS:
            try:
                return await interaction.response.send_modal(ContentModal(key))
            except Exception as e:
                log("content modal error:", repr(e))
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)

        await safe_send(interaction, f"❌ Bilinmeyen seçim: {key}", ephemeral=True)
...
#                   MASS SELECT & VIEW
# =========================================================
class MassSheetModal(discord.ui.Modal):
    """Mass content için sheet modal - default mount Swiftclaw"""
    def __init__(self, sheet_key: str):
        self.sheet_key = (sheet_key or "").strip()
        ...
        if self.sheet_key.startswith("dyn:"):
            ...
            cfg = DYNAMIC_SHEETS.get(k)
            if cfg:
                title = (cfg.get("name") or "Mass")[:45]

        super().__init__(title=title)

        self.time = discord.ui.TextInput(label="Saat", required=False, placeholder="00:00", max_length=40)
        self.toplanma = discord.ui.TextInput(label="Toplanma", required=False, placeholder="Martlock", max_length=100)
        self.binek = discord.ui.TextInput(label="Binek", required=False, placeholder=DEFAULT_MOUNT_MASS, max_length=60)
        self.ayar = discord.ui.TextInput(label="Ayar", required=False, placeholder=DEFAULT_AYAR_FALLBACK, max_length=60)
        self.add_item(self.time)
        self.add_item(self.toplanma)
        self.add_item(self.binek)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            if self.sheet_key.startswith("dyn:"):
                ...
                # Mass için default mount
                binek_val = self.binek.value.strip() if self.binek.value and self.binek.value.strip() else DEFAULT_MOUNT_MASS
...
class MassSelect(discord.ui.Select):
    """Mass content için select menu - JSON'daki 'mass' kategorisi + dynamic sheets"""
    def __init__(self):
        opts: List[discord.SelectOption] = []

        # content_templates.json'dan "mass" kategorisindekiler
        mass_keys = [k for k in _TEMPLATE_ORDER if _TEMPLATE_CATEGORY.get(k, "content") == "mass"]
        for k in mass_keys[:20]:
            tpl = PRESETS.get(k)
            if not tpl:
                continue
            emoji = _TEMPLATE_EMOJIS.get(k, "⚔️")
            label = (tpl.thread_name or tpl.title or k)[:100]
            opts.append(discord.SelectOption(label=label[:100], value=f"preset_mass:{k}", emoji=emoji if len(emoji) <= 2 else None))

        # dynamic sheets - sadece "mass" tipindekiler
        try:
            dyn_items = sorted(
                [(k, v) for k, v in DYNAMIC_SHEETS.items() if v.get("type", "content") == "mass"],
                key=lambda kv: (kv[1].get("name", kv[0]).lower(), kv[0].lower())
            )
        except Exception:
            dyn_items = []

        remaining = max(0, 24 - len(opts))
        for k, cfg in dyn_items[:remaining]:
            name = (cfg.get("name") or k).strip() or k
            emoji = (cfg.get("emoji") or "⚔️").strip() or "⚔️"
            opts.append(discord.SelectOption(label=name[:100], value=f"mass_dyn:{k}", emoji=emoji))

        if not opts:
            opts.append(discord.SelectOption(label="Mass template yok", value="none", emoji="❌"))

        super().__init__(placeholder="👇 Mass seç", min_values=1, max_values=1, options=opts[:25])

    async def callback(self, interaction: discord.Interaction):
        key = (self.values[0] or "").strip()

        if key == "none":
            return await safe_send(interaction, "❌ Henüz mass template eklenmemiş.\nPanel'den Content Yöneticisi > Şablon Ekle > Kategori: Mass ile ekleyebilirsin.", ephemeral=True)

        if key.startswith("preset_mass:"):
            preset_key = key.split(":", 1)[1].strip()
            try:
                return await interaction.response.send_modal(ContentModal(preset_key))
            except Exception as e:
                log("preset mass modal error:", repr(e))
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)

        if key.startswith("mass_dyn:"):
            dyn_key = key.split(":", 1)[1].strip()
            try:
                return await interaction.response.send_modal(MassSheetModal(f"dyn:{dyn_key}"))
            except Exception as e:
                log("mass dyn modal error:", repr(e))
                return await safe_send(interaction, "❌ Açılmadı.", ephemeral=True)

        await safe_send(interaction, "❌ Bilinmeyen seçim.", ephemeral=True)


class MassView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=120)
        self.add_item(MassSelect())
...
class CallidusBot(discord.Client):
    def __init__(self):
        ...
        # NOT: Voice tracking ayrı voice_tracker_bot.py tarafından yapılıyor
    ...
    async def setup_hook(self):
        ...
        # Activity inactivity check task - DEVRE DIŞI
    ...
    async def on_ready(self):
        ...
        log(f"📈 Puan Sync: interval={ACTIVITY_SYNC_INTERVAL}s")
        
        # Activity sync loop başlat
        self.loop.create_task(_activity_sync_loop(self))
        
        # Müzik indirme temp dosya temizlik loop'u
        self.loop.create_task(_music_cleanup_downloads())

        # Panel bridge loop başlat
        if _panel_bridge:
            self.loop.create_task(_panel_bridge.start())
        
        # Templates yeniden yükle (başlangıçta JSON hazır olsun)
        reload_presets()

    # NOT: Voice tracking ayrı voice_tracker_bot.py tarafından yapılıyor.
    # Ana bot sadece activity_state.json'dan voice_minutes değerini okuyup Sheet'e yazıyor.
...
# Panel bridge instance
_panel_bridge = _PanelBridge(bot, BASE_DIR) if _PANEL_BRIDGE_OK else None
...
async def create_event_in_channel(
    ...
    EVENTS[msg.id] = state
    if _panel_bridge: _panel_bridge.flush_active_events()
...
@bot.tree.command(name="mass", description="Mass content seçip event oluşturur.", guild=discord.Object(id=GUILD_ID))
async def mass_cmd(interaction: discord.Interaction):
    try:
        await interaction.response.send_message("👇", ephemeral=True, view=MassView())
    except Exception as e:
        log("/mass response error:", repr(e))
        try:
            await interaction.followup.send("👇", ephemeral=True, view=MassView())
        except Exception as e2:
            log("/mass followup error:", repr(e2))


# =========================================================
#                 CONTENT CLOSE (RIGHT CLICK)
# =========================================================
@bot.tree.context_menu(name="Content'i Kapat", guild=discord.Object(id=GUILD_ID))
async def content_close_menu(interaction: discord.Interaction, message: discord.Message):
    """Right-click a content message -> close it and log participants to Sheet + JSON + Content Tab."""
    await safe_defer(interaction, ephemeral=True)
    ...
    # Normal event
    state = EVENTS.get(message.id)
    sheet_state = None
    try:
        sheet_state = SHEET_EVENTS.get(message.id)  # type: ignore
    except Exception:
        sheet_state = None

    if not state and not sheet_state:
        return await safe_send(interaction, "❌ Bu mesaj bir content event değil (state bulunamadı).", ephemeral=True)

    # Permission
    if state:
        if not is_event_staff(interaction, state):
            return await safe_send(interaction, "❌ Yetki yok.", ephemeral=True)
    else:
        # Sheet event: reuse staff rule (owner/admin)
        if not interaction.guild:
            return await safe_send(interaction, "❌ Guild yok.", ephemeral=True)
        member = interaction.guild.get_member(interaction.user.id)
        perms_ok = bool(member and (member.guild_permissions.administrator or member.guild_permissions.manage_guild or member.guild_permissions.manage_channels))
        if not perms_ok and getattr(sheet_state, "owner_id", 0) != interaction.user.id:
            return await safe_send(interaction, "❌ Yetki yok.", ephemeral=True)

    # Collect participants (CURRENT roster only)
    participant_ids: List[int] = []
    content_name = ""
    time_tr = ""
    time_utc = ""
    ...
    if state:
        ...
        # Sheet event
        content_name = (getattr(sheet_state, "thread_name", "") or getattr(sheet_state, "title", "") or "SHEET CONTENT").strip()
        time_tr = getattr(sheet_state, "time_tr", "") or ""
        time_utc = getattr(sheet_state, "time_utc", "") or ""
        # sheet_state.user_slot: user_id -> slot_id; current participants are keys
        try:
    ...
    entry = {
        "closed_at_utc": datetime.now(UTC_TZ).isoformat(),
        "closed_at_tr": datetime.now(TR_TZ).isoformat(),
        "date_tr": datetime.now(TR_TZ).strftime("%Y-%m-%d"),
        ...
        "channel_id": int(message.channel.id) if message.channel else 0,
        "closed_by": int(interaction.user.id),
    }

    # Content Log'a ekle ve katılımcıları işaretle
    content_log_result = ""
    try:
        if GOOGLE_CREDS_JSON and ACTIVITY_SHEET_ID:
            # Content Log'a yeni sütun ekle (2 sütun: loot + tick)
            date_str = datetime.now(TR_TZ).strftime("%Y-%m-%d")
            loot_col, tick_col = await _add_content_to_log(content_name, date_str, time_tr or "")
            
            # Katılımcıları işaretle (tick sütununa)
            if tick_col and names:
                marked = await _mark_content_participation(tick_col, names)
                content_log_result = f"\n📋 Content Log: {marked}/{len(names)} katılımcı işaretlendi"
    except Exception as e:
        log(f"[CONTENT LOG] Hata: {e}")
        content_log_result = f"\n⚠️ Content Log hatası: {e}"
    
    # Puan Log'a content sayısını yaz (günlük content takibi)
    puan_log_result = ""
    try:
        if GOOGLE_CREDS_JSON and ACTIVITY_SHEET_ID:
            # Content sayısını Puan Log'a yaz (+1)
            updated = await _write_content_count_to_puan_log(names)
            puan_log_result = f"\n📊 Puan Log: {updated} üyenin content sayısı güncellendi"
    except Exception as e:
        log(f"[PUAN LOG] Hata: {e}")
        puan_log_result = f"\n⚠️ Puan Log hatası: {e}"

    # Disable interaction on the event message (keep embed as-is, just remove view)
    try:
        await message.edit(view=None)
    except Exception:
        pass

    # Clean state + reminder task if exists
    try:
        if message.id in EVENTS:
            EVENTS.pop(message.id, None)
        if message.id in SHEET_EVENTS:
            SHEET_EVENTS.pop(message.id, None)
            _save_sheet_events()  # Kaydet (bot restart için)
        if hasattr(interaction.client, "content_reminders"):
            task = getattr(interaction.client, "content_reminders", {}).pop(message.id, None)
            if task:
                try:
                    task.cancel()
                except Exception:
    ...
    if _panel_bridge: _panel_bridge.flush_active_events_force()

    return await safe_send(
        interaction,
        f"✅ Kapandı ve loglandı: **{content_name}** | Katılımcı: **{len(participant_ids)}**{content_log_result}{puan_log_result}",
        ephemeral=True
    )
...
@bot.tree.command(name="sheetekle", description="Sheet'i /content veya /mass'e ekler.", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(
    isim="Menüde gözükecek ad",
    link="Google Sheet linki veya ID",
    tab="Tab adı",
    tip="Hangi menüye eklensin? (content veya mass)",
    emoji="Emoji (opsiyonel)"
)
@app_commands.choices(tip=[
    app_commands.Choice(name="content", value="content"),
    app_commands.Choice(name="mass", value="mass"),
])
async def sheetekle_cmd(
    ...
    link: str,
    tab: str,
    tip: app_commands.Choice[str] = None,
    ...
    sheet_type = tip.value if tip else "content"
    ...
    DYNAMIC_SHEETS[key] = {"name": isim, "sheet_id": sheet_id, "tab": tab, "emoji": emoji, "type": sheet_type}
    _save_dynamic_sheets()

    menu_name = "/content" if sheet_type == "content" else "/mass"
    return await safe_send(interaction, f"✅ Eklendi: **{isim}** (`{key}`) → {menu_name}", ephemeral=True)
...
async def sheetliste_cmd(interaction: discord.Interaction):
    ...
    lines = []
    for k, v in sorted(DYNAMIC_SHEETS.items(), key=lambda kv: (kv[1].get("type", "content"), kv[1].get("name", kv[0]).lower())):
        sheet_type = v.get("type", "content")
        type_icon = "📋" if sheet_type == "content" else "⚔️"
        lines.append(f"{type_icon} `{k}` → {v.get('name', k)}  ({v.get('tab','')}) [{sheet_type}]")
    msg = "\n".join(lines)
    if len(msg) > 1800:
        msg = msg[:1800] + "\n…"
...
YTDL_OPTIONS = {
    # Daha esnek format: herhangi bir audio formatını kabul et
...
def _build_ffmpeg_headers(info: dict) -> str:
    """YouTube 403 hatası için ffmpeg header'ları oluşturur."""
    url = info.get('url', '')
    
    # URL'deki client tipine göre User-Agent seç
    # c=ANDROID veya c=ANDROID_TESTSUITE varsa Android UA kullan
    if 'c=ANDROID' in url:
    ...
    # ffmpeg -headers formatı: "Key: Value\r\nKey2: Value2\r\n"
    return "".join([f"{k}: {v}\r\n" for k, v in h.items()])

def _get_ffmpeg_options_with_headers(info: dict) -> dict:
    """Header'lı FFMPEG options döndürür."""
    headers = _build_ffmpeg_headers(info)
    
    # Header'ları escape et (çift tırnak için)
    headers_escaped = headers.replace('"', '\\"')
    
    before = f'-nostdin -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -headers "{headers_escaped}"'
...
# ---- İndirme modu için dizin ----
import os as _os2
_MUSIC_DL_DIR = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '_music_downloads')
_os.makedirs(_MUSIC_DL_DIR, exist_ok=True)

# İndirme temp dosya temizlik ayarları
_MUSIC_DL_MAX_AGE = 3600  # 1 saatten eski dosyaları sil

async def _music_cleanup_downloads():
    """Eski indirilen müzik dosyalarını periyodik olarak temizler."""
    while True:
        await asyncio.sleep(1800)  # 30 dakikada bir
        try:
            if _os.path.exists(_MUSIC_DL_DIR):
                now = time.time()
                cleaned = 0
                for f in _os.listdir(_MUSIC_DL_DIR):
                    fp = _os.path.join(_MUSIC_DL_DIR, f)
                    if _os.path.isfile(fp):
                        age = now - _os.path.getmtime(fp)
                        if age > _MUSIC_DL_MAX_AGE:
                            try:
                                _os.unlink(fp)
                                cleaned += 1
                            except Exception:
                                pass
                if cleaned:
                    _music_log(f"Temp temizlik: {cleaned} eski dosya silindi")
        except Exception as e:
            _music_log(f"Temp temizlik hatası: {e}")


async def _music_search(query: str) -> Optional[Dict[str, Any]]:
    """Şarkıyı indirir ve lokal dosya olarak döndürür.
    Stream yerine indirme kullanır çünkü YouTube 403 hatası veriyor."""
    if not YTDLP_OK:
        return None
    ...
    def _download():
        ...
        output_template = _os.path.join(_MUSIC_DL_DIR, f"{dl_id}_%(title)s.%(ext)s")
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                if is_url:
                    _music_log(f"URL indiriliyor: {query}")
                    info = ydl.extract_info(query, download=True)
                else:
                    _music_log(f"Aranıyor ve indiriliyor: {query}")
                ...
                pattern = _os.path.join(_MUSIC_DL_DIR, f"{dl_id}_*")
                found_files = glob.glob(pattern)
                
                if not found_files:
                    _music_log(f"İndirilen dosya bulunamadı! Pattern: {pattern}")
                    return None
                
                # mp3 varsa onu tercih et
                file_path = None
                ...
                if not file_path:
                    for f in found_files:
                        if f.endswith(('.m4a', '.webm', '.ogg', '.opus', '.mp4', '.wav')):
                            file_path = f
                            break
                if not file_path:
                ...
                _music_log(f"Dosya indirildi: {_os.path.basename(file_path)} ({file_size} bytes)")
                
                if file_size < 1000:
                    _music_log(f"Dosya çok küçük ({file_size} bytes), muhtemelen hatalı!")
                    # Temizle
                    for f in found_files:
                        try: _os.unlink(f)
                        except: pass
            ...
            _music_log(f"İndirme hatası: {repr(e)}")
    ...
    try:
        ...
        _music_log("İndirme timeout (60sn)!")
        return None
    except Exception as e:
        _music_log(f"Async hata: {repr(e)}")
        return None
...
async def _music_play_next_local(bot: "CallidusBot", guild_id: int):
    """Sıradaki şarkıyı çalar (local dosya desteği ile)."""
    ...
    try:
        ...
        if song.get('is_local_file') and song.get('local_path'):
            local_path = song['local_path']
            _music_log(f"Local dosya çalınıyor: {local_path}")
            
            if not os.path.exists(local_path):
                _music_log(f"Local dosya bulunamadı: {local_path}")
                await _music_play_next_local(bot, guild_id)
                return
            
            # Local dosya için basit ffmpeg options
            ffmpeg_opts = {
                'before_options': '-nostdin',
                'options': '-vn',
            }
            
            source = discord.FFmpegPCMAudio(local_path, **ffmpeg_opts)
            volume = bot.music_volume.get(guild_id, MUSIC_DEFAULT_VOLUME)
            source = discord.PCMVolumeTransformer(source, volume=volume)
            
            def after_playing_local(error):
                if error:
                    _music_log(f"Local oynatma hatası: {repr(error)}")
                # Geçici dosyayı sil (sadece delete_after True ise)
                if song.get('delete_after', True):
                    try:
                        if os.path.exists(local_path):
                            os.unlink(local_path)
                            _music_log(f"Geçici dosya silindi: {local_path}")
                    except Exception as e:
                        _music_log(f"Geçici dosya silinemedi: {e}")
                # Sonraki şarkıya geç
                asyncio.run_coroutine_threadsafe(_music_play_next_local(bot, guild_id), bot.loop)
            
            vc.play(source, after=after_playing_local)
            _music_log(f"Local dosya çalıyor: {song.get('title')} (guild={guild_id})")
        else:
            # YouTube/URL şarkısı - indir ve lokal olarak çal
            ...
            # İndirilen lokal dosya mı?
            if fresh_info.get('is_local_file') and fresh_info.get('local_path'):
                dl_path = fresh_info['local_path']
                _music_log(f"İndirilen dosya çalınıyor: {_os.path.basename(dl_path)}")
                
                if not os.path.exists(dl_path):
                    _music_log(f"İndirilen dosya bulunamadı: {dl_path}")
                    await _music_play_next_local(bot, guild_id)
                    return
                
                # now_playing bilgilerini güncelle
                bot.music_now_playing[guild_id] = {**song, **fresh_info}
                
                ffmpeg_opts = {'before_options': '-nostdin', 'options': '-vn'}
                source = discord.FFmpegPCMAudio(dl_path, **ffmpeg_opts)
                volume = bot.music_volume.get(guild_id, MUSIC_DEFAULT_VOLUME)
                source = discord.PCMVolumeTransformer(source, volume=volume)
                
                def after_playing_dl(error):
                    if error:
                        _music_log(f"Oynatma hatası: {repr(error)}")
                    # İndirilen dosyayı sil
                    if fresh_info.get('delete_after', True):
                        try:
                            if os.path.exists(dl_path):
                                os.unlink(dl_path)
                                _music_log(f"İndirilen dosya silindi: {_os.path.basename(dl_path)}")
                        except Exception as e:
                            _music_log(f"Dosya silinemedi: {e}")
                    asyncio.run_coroutine_threadsafe(_music_play_next_local(bot, guild_id), bot.loop)
                
                vc.play(source, after=after_playing_dl)
                _music_log(f"Çalıyor (indirilen): {fresh_info.get('title')} (guild={guild_id})")
            elif fresh_info.get('url'):
                # Stream URL varsa (SoundCloud gibi YouTube olmayan kaynaklar)
                _music_log(f"Stream çalınıyor: {fresh_info.get('title')}")
                
                ffmpeg_opts = _get_ffmpeg_options_with_headers(fresh_info)
                source = discord.FFmpegPCMAudio(fresh_info['url'], **ffmpeg_opts)
                volume = bot.music_volume.get(guild_id, MUSIC_DEFAULT_VOLUME)
                source = discord.PCMVolumeTransformer(source, volume=volume)
                ...
                _music_log(f"Çalıyor: {song.get('title')} (guild={guild_id})")
            else:
                _music_log(f"Ne URL ne dosya bulunamadı, atlıyorum: {song.get('title')}")
                await _music_play_next_local(bot, guild_id)
    except Exception as e:
...
async def _music_play_next(bot: "CallidusBot", guild_id: int):
    """Sıradaki şarkıyı çalar. İndirme modu için _music_play_next_local'a yönlendirir."""
    await _music_play_next_local(bot, guild_id)
...
if YTDLP_OK:
    ...
    @bot.tree.command(name="cal", description="YouTube'dan müzik çalar.", guild=discord.Object(id=GUILD_ID))
    @app_commands.describe(sorgu="YouTube şarkı linki veya şarkı adı")
    ...
    @bot.tree.command(name="calsunucu", description="Bot klasöründeki ses dosyasını çalar.", guild=discord.Object(id=GUILD_ID))
    @app_commands.describe(dosya="Dosya adı (örn: sarki.mp3)")
    async def music_play_server_file_cmd(interaction: discord.Interaction, dosya: str):
        await interaction.response.defer()
        
        # Bot klasörü
        bot_dir = Path(__file__).parent
        file_path = bot_dir / dosya
        
        # Dosya var mı kontrol et
        if not file_path.exists():
            return await interaction.followup.send(f"❌ Dosya bulunamadı: `{dosya}`")
        
        # Dosya uzantısı kontrolü
        filename = dosya.lower()
        if not filename.endswith(('.mp3', '.mp4', '.m4a', '.wav', '.ogg', '.webm')):
            return await interaction.followup.send("❌ Sadece ses dosyaları desteklenir! (mp3, mp4, m4a, wav, ogg, webm)")
        ...
        # Şarkı bilgisi oluştur
        song = {
            'title': file_path.stem,  # Uzantısız dosya adı
            'url': str(file_path),
            'duration': 0,
            'thumbnail': None,
            'webpage_url': None,
            ...
            'delete_after': False,  # Bot klasöründeki dosya silinmesin
        }
...
async def on_ready():
    ...
    # Loot persistent views
    _register_loot_views(bot)
    log(f"💰 Loot system ready (channel={LOOT_CHANNEL_ID})")
    
    # Content event persistent views
    _load_sheet_events()
    _register_sheet_event_views(bot)
    log(f"📋 Content events ready ({len(SHEET_EVENTS)} aktif)")
    
    # Puan Log auto sync loop başlat
    bot.loop.create_task(_puan_log_auto_sync_loop(bot))
    log(f"✅ Puan Log auto sync başlatıldı (her {PUAN_LOG_AUTO_SYNC_INTERVAL}s)")
...
# =========================================================
#                 YENİ PUAN SİSTEMİ & GOOGLE SHEETS SYNC
# =========================================================
"""
YENİ PUAN SİSTEMİ:
- Voice verisi Voice Tracker Bot'tan okunur (activity_state.json)
- 1 puan = 2 dakika (0.5 puan/dakika)
- Günlük max 60 puan (120 dakika)
- TR 00:00'da reset
- -3000 uyarı, -5000 kick (otomatik değil)
"""
ACTIVITY_SHEET_ID = os.getenv("ACTIVITY_SHEET_ID", "1sXmJEq9SYSSxn8hju-4qiuuRnMzPKlLduGuuXLnrFEg")
ACTIVITY_SHEET_TAB = os.getenv("ACTIVITY_SHEET_TAB", "Activity Balance")
ACTIVITY_SYNC_INTERVAL = int(os.getenv("ACTIVITY_SYNC_INTERVAL", "600"))  # 10 dakika (saniye)
ACTIVITY_MEMBER_ROLE_ID = int(os.getenv("ACTIVITY_MEMBER_ROLE_ID", "1419663333874729121"))  # Sadece bu role sahip olanlar
PUAN_LOG_AUTO_SYNC_INTERVAL = int(os.getenv("PUAN_LOG_AUTO_SYNC_INTERVAL", "3600"))  # 1 saat (saniye) - Puan Log auto sync

async def _sync_puan_to_sheet(bot_client):
    """
    Puan verilerini Google Sheet'e sync eder.
    Voice verisini activity_state.json'dan okur, puanları hesaplar ve yazar.
    Ayrıca eşik kontrollerini yapar.
    """
    if not GOOGLE_CREDS_JSON or not ACTIVITY_SHEET_ID:
        log("[SYNC] Google Sheets yapılandırması eksik, sync atlandı.")
        return
    
    try:
        # Voice verisini activity_state.json'dan oku
        activity_state = _load_activity_state()
        activity_users = activity_state.get("users", {})
        
        # Puan state'ini yükle ve günlük reset kontrolü yap
        puan_state = _load_puan_state()
        reset_done = _check_and_reset_daily(puan_state)
        if reset_done:
            log("[SYNC] Günlük puan reset yapıldı (TR 00:00)")
        
        # Guild'i al
        guild = bot_client.get_guild(GUILD_ID)
        if not guild:
            log("[SYNC] Guild bulunamadı.")
            return
        
        # Member rolünü bul
        member_role = guild.get_role(ACTIVITY_MEMBER_ROLE_ID)
        if not member_role:
            log(f"[SYNC] Member rolü bulunamadı: {ACTIVITY_MEMBER_ROLE_ID}")
            return
        
        # Sheet'e yazılacak verileri hazırla
        rows = []
        now_str = datetime.now(TR_TZ).strftime("%Y-%m-%d %H:%M")
        warnings_to_send = []  # (member, warn_type, total_points)
        
        # Member rolü olan TÜM üyeleri döngüle
        for member in guild.members:
            try:
                if member.bot:
                    continue
                ...
                # IGN (Albion ismi)
                ign = ""
                if user_id in _player_links:
                    ign = _player_links[user_id].albion_name
                
                # Voice dakikalarını activity_state'ten oku
                voice_data = activity_users.get(user_id_str, {})
                current_voice_minutes = voice_data.get("voice_minutes", 0) or 0
                
                # Content katılımlarını activity_state'ten oku
                content_joins = voice_data.get("content_joins", 0) or 0
                
                # Puanı güncelle (voice'tan)
                result = _update_user_points_from_voice(user_id, current_voice_minutes)
                ...
                # Eşik kontrolü
                threshold_check = _check_puan_thresholds(user_id)
                
                # Uyarı gerekiyor mu?
                if threshold_check["needs_kick_warning"] and not threshold_check["already_kick_warned"]:
                    warnings_to_send.append((member, "kick", total_points))
                    _mark_user_warned(user_id, "kick")
                elif threshold_check["needs_warning"] and not threshold_check["already_warned"]:
                    warnings_to_send.append((member, "warning", total_points))
                    _mark_user_warned(user_id, "warning")
                
                # Durum belirleme
                if total_points <= PUAN_KICK_THRESHOLD:
                    status = "🔴 KICK SEVİYESİ"
                elif total_points <= PUAN_WARNING_THRESHOLD:
                    status = "🟠 UYARI"
                elif total_points < 0:
                    status = "🟡 Negatif"
                else:
                ...
                rows.append([
                    user_id_str,                    # A: Discord ID
                    discord_name,                   # B: Discord Name
                    ign,                            # C: IGN
                    current_voice_minutes,          # D: Toplam Voice (dk)
                    content_joins,                  # E: Toplam Content
                    round(total_points, 1),         # F: Toplam Puan
                    status,                         # G: Durum
                    now_str                         # H: Son Güncelleme
                ])
            except Exception as e:
                log(f"[SYNC] Kullanıcı işleme hatası {member.id}: {e}")
                continue
        
        if not rows:
            log("[SYNC] Yazılacak satır yok.")
            return
        ...
        # Google Sheets'e yaz
        client = _gs_authorize_sync()
        sheet = client.open_by_key(ACTIVITY_SHEET_ID)
        worksheet = sheet.worksheet(ACTIVITY_SHEET_TAB)
        
        # Başlıklar (Toplam Content eklendi)
        headers = [
            "Discord ID", "Discord Name", "IGN", 
            "Toplam Voice (dk)", "Toplam Content", "Toplam Puan", "Durum", "Son Güncelleme"
        ]
        
        try:
            worksheet.update(values=[headers], range_name="A1:H1")
            worksheet.batch_clear(["A2:I500", "I1"])
            
            if rows:
                cell_range = f"A2:H{len(rows) + 1}"
                worksheet.update(values=rows, range_name=cell_range)
            
            log(f"[SYNC] ✅ {len(rows)} üye Sheet'e yazıldı.")
        except Exception as e:
            log(f"[SYNC] Sheet yazma hatası: {e}")
        
        # Uyarıları gönder
        for member, warn_type, points in warnings_to_send:
            await _send_puan_warning(bot_client, member, warn_type, points)
            
    except Exception as e:
        log(f"[SYNC] Genel hata: {e}")

async def _send_puan_warning(bot_client, member: discord.Member, warn_type: str, total_points: float):
    """Puan uyarısı gönderir (DM ve/veya log kanalı)."""
    try:
        if warn_type == "kick":
            title = "🚨 KRİTİK PUAN UYARISI"
            description = (
                f"**{member.display_name}**, puanın kritik seviyeye düştü!\n\n"
                f"**Toplam Puan:** {total_points:.1f}\n"
                f"**Eşik:** {PUAN_KICK_THRESHOLD}\n\n"
                "⚠️ Bu seviyede guild'den çıkarılma riski var!\n"
                "Lütfen voice aktiviteni artır."
            )
            color = 0xFF0000
        else:
            title = "⚠️ PUAN UYARISI"
            description = (
                f"**{member.display_name}**, puanın uyarı seviyesine düştü.\n\n"
                f"**Toplam Puan:** {total_points:.1f}\n"
                f"**Eşik:** {PUAN_WARNING_THRESHOLD}\n\n"
                "Voice aktiviteni artırarak puanını yükseltebilirsin."
            )
            color = 0xFFA500
        
        embed = discord.Embed(title=title, description=description, color=color)
        embed.set_footer(text=f"1 puan = 2 dakika voice • Günlük max: 60 puan")
        
        # DM gönder
        try:
            await member.send(embed=embed)
            log(f"[PUAN] {warn_type.upper()} DM gönderildi: {member.display_name}")
        except Exception as e:
            log(f"[PUAN] DM gönderilemedi ({member.display_name}): {e}")
        
        # Log kanalına gönder
        if PUAN_LOG_CHANNEL_ID:
            try:
                channel = bot_client.get_channel(PUAN_LOG_CHANNEL_ID)
                if channel:
                    admin_embed = discord.Embed(
                        title=f"{'🚨 KICK UYARISI' if warn_type == 'kick' else '⚠️ PUAN UYARISI'}",
                        description=f"**Üye:** {member.mention}\n**Puan:** {total_points:.1f}",
                        color=color
                    )
                    await channel.send(embed=admin_embed)
            except Exception as e:
                log(f"[PUAN] Log kanalına gönderilemedi: {e}")
                
    except Exception as e:
        log(f"[PUAN] Uyarı gönderme hatası: {e}")

async def _puan_sync_loop(bot_client):
    """Belirli aralıklarla puanları sync eder ve reset kontrolü yapar."""
    await bot_client.wait_until_ready()
    log(f"[SYNC] Puan sync loop başladı (her {ACTIVITY_SYNC_INTERVAL} saniye)")
    
    while not bot_client.is_closed():
        try:
            await _sync_puan_to_sheet(bot_client)
        except Exception as e:
            log(f"[SYNC] Loop hatası: {e}")
        
        await asyncio.sleep(ACTIVITY_SYNC_INTERVAL)

# Eski fonksiyon ismi için uyumluluk
async def _sync_activity_to_sheet(bot_client):
    """Eski fonksiyon ismi - yeni sisteme yönlendirir."""
    await _sync_puan_to_sheet(bot_client)

async def _activity_sync_loop(bot_client):
    """Eski fonksiyon ismi - yeni sisteme yönlendirir."""
    await _puan_sync_loop(bot_client)

@bot.tree.command(name="puan", description="Kendi puan durumunu gösterir.", guild=discord.Object(id=GUILD_ID))
async def puan_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    user_id = interaction.user.id
    
    # Voice verisini activity_state'ten oku
    activity_state = _load_activity_state()
    voice_data = activity_state.get("users", {}).get(str(user_id), {})
    current_voice_minutes = voice_data.get("voice_minutes", 0) or 0
    
    # Puan verisini puan_state'ten oku
    puan_state = _load_puan_state()
    _check_and_reset_daily(puan_state)  # Günlük reset kontrolü
    user_puan = _get_user_puan_data(puan_state, user_id)
    
    total_points = user_puan.get("total_points", 0.0) or 0.0
    daily_points = user_puan.get("daily_points", 0.0) or 0.0
    daily_minutes = user_puan.get("daily_minutes_counted", 0) or 0
    
    # Durum belirleme
    if total_points <= PUAN_KICK_THRESHOLD:
        status = "🔴 KRİTİK - Kick Riski"
    ...
    embed = discord.Embed(title="📊 Puan Durumun", color=status_color)
    
    # Günlük bilgiler
    remaining_daily = max(0, PUAN_DAILY_MAX - daily_points)
    remaining_minutes = int(remaining_daily / PUAN_PER_MINUTE)
    
    embed.add_field(
        name="📅 Bugün",
        value=f"**{daily_points:.1f}** / {PUAN_DAILY_MAX} puan\n"
              f"🎤 {daily_minutes} dk sayıldı\n"
              f"Kalan: {remaining_daily:.1f} puan ({remaining_minutes} dk)",
        inline=True
    )
    
    embed.add_field(
        name="📈 Toplam",
        value=f"**{total_points:.1f}** puan\n"
              f"🎤 {current_voice_minutes} dk (tüm zamanlar)",
        inline=True
    )
    ...
    embed.set_footer(text=f"1 puan = 2 dk voice • Günlük max: {PUAN_DAILY_MAX} puan • Uyarı: {PUAN_WARNING_THRESHOLD} • Kick: {PUAN_KICK_THRESHOLD}")
    
    await safe_send(interaction, embed=embed, ephemeral=True)

@bot.tree.command(name="puan-top", description="En yüksek puanlı üyeleri gösterir.", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(limit="Kaç kişi gösterilsin (varsayılan: 10)")
async def puan_top_cmd(interaction: discord.Interaction, limit: int = 10):
    ...
    puan_list = []
    for user_id_str, user_data in puan_state.get("users", {}).items():
        total_points = user_data.get("total_points", 0.0) or 0.0
        
        # Voice dakikasını activity_state'ten al
        voice_minutes = activity_state.get("users", {}).get(user_id_str, {}).get("voice_minutes", 0) or 0
        
        puan_list.append((int(user_id_str), total_points, voice_minutes))
    
    puan_list.sort(key=lambda x: x[1], reverse=True)
    
    embed = discord.Embed(title="🏆 Puan Sıralaması", color=0xFFD700)
    
    lines = []
    for i, (user_id, puan, voice) in enumerate(puan_list[:limit], 1):
        ...
        # Durum ikonu
        if puan <= PUAN_KICK_THRESHOLD:
            status_icon = "🔴"
        elif puan <= PUAN_WARNING_THRESHOLD:
            status_icon = "🟠"
        elif puan < 0:
            status_icon = "🟡"
        else:
        ...
        lines.append(f"{medal} {name} — **{puan:.1f}** puan {status_icon} (🎤{voice}dk)")
    
    embed.description = "\n".join(lines) if lines else "Henüz veri yok."
    embed.set_footer(text=f"Toplam {len(puan_list)} üye • 1 puan = 2 dk voice")
    
    await safe_send(interaction, embed=embed)

@bot.tree.command(name="sync", description="Activity verilerini Google Sheet'e sync eder.", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def sync_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    try:
        await _sync_activity_to_sheet(interaction.client)
        await safe_send(interaction, "✅ Activity verileri Google Sheet'e sync edildi!", ephemeral=True)
    except Exception as e:
        await safe_send(interaction, f"❌ Sync hatası: {e}", ephemeral=True)

@bot.tree.command(name="puan-ayarla", description="Bir üyenin toplam puanını manuel ayarlar.", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(uye="Puanı ayarlanacak üye", puan="Ayarlanacak toplam puan değeri")
@app_commands.default_permissions(administrator=True)
async def puan_ayarla_cmd(interaction: discord.Interaction, uye: discord.Member, puan: float):
    await safe_defer(interaction, ephemeral=True)
    
    _set_user_total_points(uye.id, puan)
    
    # Eğer puan eşiklerin üstüne çıktıysa uyarıları temizle
    if puan > PUAN_WARNING_THRESHOLD:
        _clear_user_warnings(uye.id)
    
    await safe_send(
        interaction, 
        f"✅ **{uye.display_name}** puanı ayarlandı:\n⭐ Toplam Puan: **{puan:.1f}**", 
        ephemeral=True
    )

@bot.tree.command(name="puan-sifirla", description="Tüm puanları sıfırlar (DİKKAT!).", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(onay="Sıfırlamak için 'EVET' yaz")
@app_commands.default_permissions(administrator=True)
async def puan_sifirla_cmd(interaction: discord.Interaction, onay: str = ""):
    await safe_defer(interaction, ephemeral=True)
    
    if onay.upper() != "EVET":
        return await safe_send(
            interaction, 
            "⚠️ **DİKKAT:** Bu işlem TÜM puanları sıfırlayacak!\n"
            "Onaylamak için komutu `onay: EVET` parametresiyle tekrar çalıştır.", 
            ephemeral=True
        )
    
    puan_state = _load_puan_state()
    for user_id_str in puan_state.get("users", {}):
        puan_state["users"][user_id_str]["total_points"] = 0.0
        puan_state["users"][user_id_str]["daily_points"] = 0.0
        puan_state["users"][user_id_str]["daily_minutes_counted"] = 0
    
    puan_state["warned_users"] = []
    puan_state["kick_warned_users"] = []
    _save_puan_state(puan_state)
    
    await safe_send(interaction, "✅ Tüm puanlar sıfırlandı!", ephemeral=True)

@bot.tree.command(name="puan-debug", description="Puan sistemi debug bilgisi.", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def puan_debug_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    puan_state = _load_puan_state()
    activity_state = _load_activity_state()
    
    user_count = len(puan_state.get("users", {}))
    warned_count = len(puan_state.get("warned_users", []))
    kick_warned_count = len(puan_state.get("kick_warned_users", []))
    last_reset = puan_state.get("last_reset_date", "Hiç")
    today = _get_today_tr()
    
    activity_users = len(activity_state.get("users", {}))
    
    embed = discord.Embed(title="🔧 Puan Sistemi Debug", color=0x9B59B6)
    embed.add_field(
        name="📊 State Bilgisi",
        value=f"Puan users: {user_count}\n"
              f"Activity users: {activity_users}\n"
              f"Uyarı alan: {warned_count}\n"
              f"Kick uyarısı: {kick_warned_count}",
        inline=True
    )
    embed.add_field(
        name="📅 Reset Bilgisi",
        value=f"Bugün: {today}\n"
              f"Son reset: {last_reset}\n"
              f"Reset gerekli: {'Evet' if last_reset != today else 'Hayır'}",
        inline=True
    )
    embed.add_field(
        name="⚙️ Ayarlar",
        value=f"Puan/dk: {PUAN_PER_MINUTE}\n"
              f"Günlük max: {PUAN_DAILY_MAX}\n"
              f"Uyarı eşiği: {PUAN_WARNING_THRESHOLD}\n"
              f"Kick eşiği: {PUAN_KICK_THRESHOLD}",
        inline=False
    )
...
# =========================================================
#     FULL SYNC (Activity Balance + Content Log + Puan Log + Voice)
# =========================================================
async def _run_full_sync(bot_client: discord.Client) -> Dict[str, Any]:
    """Tek seferde TÜM sync işlemlerini yapar.
    - Activity Balance güncelleme
    - Content Log member sync
    - Puan Log member sync
    - Bugünün (DD.MM V / DD.MM C) sütunlarını garanti oluşturur
    - Bugünün voice dakikalarını Puan Log'a yazar
    
    Rate limit: 60 istek/dakika - her adım arasında 30sn bekle
    """
    out: Dict[str, Any] = {
        "activity_balance": False,
        "content_log_added": 0, 
        "puan_log_added": 0, 
        "voice_written": 0, 
        "today": _get_today_date_str()
    ...
    if not (GOOGLE_CREDS_JSON and ACTIVITY_SHEET_ID):
        out["error"] = "GOOGLE_CREDS_JSON veya ACTIVITY_SHEET_ID ayarlı değil."
        return out

    # 1) Activity Balance: ana özet tabloyu güncelle
    try:
        await _sync_puan_to_sheet(bot_client)
        out["activity_balance"] = True
        log(f"[FULL SYNC] Activity Balance tamamlandı")
    except Exception as e:
        out["activity_balance_error"] = str(e)
        log(f"[FULL SYNC] Activity Balance hatası: {e}")
    
    # Rate limit için bekle (30 saniye)
    await asyncio.sleep(30)

    # 2) Content Log: üyeleri yaz
    try:
        out["content_log_added"] = await _sync_members_to_content_log(bot_client)
        log(f"[FULL SYNC] Content Log tamamlandı: +{out['content_log_added']}")
    except Exception as e:
        out["content_log_error"] = str(e)
        log(f"[FULL SYNC] Content Log hatası: {e}")
    
    # Rate limit için bekle (30 saniye)
    await asyncio.sleep(30)

    # 3) Puan Log: üyeleri ekle
    try:
        out["puan_log_added"] = await _sync_members_to_puan_log(bot_client)
        log(f"[FULL SYNC] Puan Log üyeler tamamlandı: +{out['puan_log_added']}")
    except Exception as e:
        out["puan_log_error"] = str(e)
        log(f"[FULL SYNC] Puan Log üyeler hatası: {e}")
    
    # Rate limit için bekle (30 saniye)
    await asyncio.sleep(30)

    # 4) Bugünün sütunlarını oluştur (V/C header)
    try:
        def _ensure_cols():
            ws = _get_puan_log_worksheet()
            _find_or_create_daily_columns(ws, out["today"])
            return True
        await run_io(_ensure_cols)
        log(f"[FULL SYNC] Günlük sütunlar oluşturuldu: {out['today']}")
    except Exception as e:
        out["cols_error"] = str(e)
        log(f"[FULL SYNC] Sütun oluşturma hatası: {e}")
    
    # Rate limit için bekle (30 saniye)
    await asyncio.sleep(30)

    # 5) Voice dakikalarını yaz
    try:
        out["voice_written"] = await _write_daily_voice_to_puan_log(bot_client)
        log(f"[FULL SYNC] Voice dakikaları yazıldı: {out['voice_written']} üye")
    except Exception as e:
        out["voice_error"] = str(e)
        log(f"[FULL SYNC] Voice yazma hatası: {e}")

    return out


async def _puan_log_auto_sync_loop(bot_client: discord.Client) -> None:
    """Saatlik otomatik sync."""
    await bot_client.wait_until_ready()
    
    # İlk açılışta 2 dakika bekle (bot başlaması için)
    await asyncio.sleep(120)
    
    # İlk sync
    try:
        res = await _run_full_sync(bot_client)
        if "error" in res:
            log(f"[AUTO SYNC] Başlatma hatası: {res['error']}")
        else:
            log(f"[AUTO SYNC] İlk sync OK | Activity:{res.get('activity_balance')} | Content+{res.get('content_log_added',0)} | Puan+{res.get('puan_log_added',0)} | Voice:{res.get('voice_written',0)}")
    except Exception as e:
        log(f"[AUTO SYNC] İlk sync exception: {e!r}")

    while not bot_client.is_closed():
        try:
            # Saatlik interval (minimum 10 dakika)
            await asyncio.sleep(max(600, int(PUAN_LOG_AUTO_SYNC_INTERVAL)))
            res = await _run_full_sync(bot_client)
            if "error" in res:
                log(f"[AUTO SYNC] Hata: {res['error']}")
            else:
                log(f"[AUTO SYNC] OK | Activity:{res.get('activity_balance')} | Content+{res.get('content_log_added',0)} | Puan+{res.get('puan_log_added',0)} | Voice:{res.get('voice_written',0)}")
        except asyncio.CancelledError:
            break
        except Exception as e:
            log(f"[AUTO SYNC] Loop error: {e!r}")
            try:
                await asyncio.sleep(300)  # Hata sonrası 5dk bekle
            except Exception:
                pass


@bot.tree.command(name="fullsync", description="Tek komutla Activity Balance + Content Log + Puan Log + Voice sync (~2.5 dk).", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def sync_all_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    # Başlangıç mesajı
    await safe_send(interaction, "⏳ Full sync başlatıldı... Bu işlem ~2.5 dakika sürebilir (rate limit nedeniyle).", ephemeral=True)
    
    try:
        res = await _run_full_sync(interaction.client)
        if "error" in res:
            return await safe_send(interaction, f"❌ Sync yapılamadı: {res['error']}", ephemeral=True)

        msg = (
            f"✅ **Full Sync tamamlandı!** ({res.get('today','')})\n\n"
            f"📈 Activity Balance: **{'✅' if res.get('activity_balance') else '❌'}**\n"
            f"📋 Content Log: **+{res.get('content_log_added',0)}** üye\n"
            f"📊 Puan Log (üyeler): **+{res.get('puan_log_added',0)}** yeni\n"
            f"🎤 Voice yazıldı: **{res.get('voice_written',0)}** üye\n"
        )
        # varsa hata detaylarını ekle
        errs = []
        for k in ("activity_balance_error","content_log_error","puan_log_error","cols_error","voice_error"):
            if k in res and res[k]:
                errs.append(f"• {k}: {res[k][:100]}")
        if errs:
            msg += "\n⚠️ **Bazı hatalar:**\n" + "\n".join(errs)
...
@bot.tree.command(name="content-log-sync", description="Content Log'u member listesiyle sync eder.", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def content_log_sync_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    try:
        count = await _sync_members_to_content_log(interaction.client)
        await safe_send(interaction, f"✅ Content Log güncellendi! {count} üye yazıldı.", ephemeral=True)
    except Exception as e:
        await safe_send(interaction, f"❌ Hata: {e}", ephemeral=True)

@bot.tree.command(name="puan-log-sync", description="Puan Log'u member listesiyle sync eder.", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def puan_log_sync_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    try:
        count = await _sync_members_to_puan_log(interaction.client)
        await safe_send(interaction, f"✅ Puan Log güncellendi! {count} yeni üye eklendi.", ephemeral=True)
    except Exception as e:
        await safe_send(interaction, f"❌ Hata: {e}", ephemeral=True)

@bot.tree.command(name="puan-log-voice", description="Bugünün voice puanlarını Puan Log'a yazar.", guild=discord.Object(id=GUILD_ID))
@app_commands.default_permissions(administrator=True)
async def puan_log_voice_cmd(interaction: discord.Interaction):
    await safe_defer(interaction, ephemeral=True)
    
    try:
        count = await _write_daily_voice_to_puan_log(interaction.client)
        today = _get_today_date_str()
        await safe_send(interaction, f"✅ Voice puanları yazıldı! ({today} V) - {count} üye güncellendi.", ephemeral=True)
    except Exception as e:
        await safe_send(interaction, f"❌ Hata: {e}", ephemeral=True)

@bot.tree.command(name="puan-log-info", description="Puan Log yapısı hakkında bilgi verir.", guild=discord.Object(id=GUILD_ID))
async def puan_log_info_cmd(interaction: discord.Interaction):
    embed = discord.Embed(
        title="📊 Puan Log Sistemi",
        description="Günlük puan takibi için Google Sheets yapısı",
        color=0x3498db
    )
    ...
    embed.add_field(
        ...
        value=(
            "**A:** Member Name (sabit)\n"
            "**B:** IGN (sabit)\n"
            "**C:** Total Points (SUM formülü)\n"
            "**D+:** Günlük sütunlar (tarih V, tarih C)"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="🎤 Voice Puanları",
        value=(
            f"• Max günlük: **60** puan\n"
            f"• 1 puan = 2 dakika\n"
            f"• Sütun: `{_get_today_date_str()} V`"
        ),
        inline=True
    )
    
    embed.add_field(
        name="📅 Content Puanları",
        value=(
            f"• Katıldı: **+{CONTENT_JOINED_POINTS}**\n"
            f"• Katılmadı: **{CONTENT_MISSED_POINTS}**\n"
            f"• Sütun: `{_get_today_date_str()} C`"
        ),
        inline=True
    )
    ...
    embed.add_field(
        name="⚙️ Komutlar",
        value=(
            "`/puan-log-sync` - Üyeleri ekle\n"
            "`/puan-log-voice` - Voice puanlarını yaz\n"
            "`/content-log-sync` - Content Log sync"
        ),
        inline=False
    )
    
    embed.set_footer(text="Content kapatıldığında puanlar otomatik yazılır")
...
# =========================================================
"""
Loot Dağıtım Sistemi - Content thread'i içinde /loot komutu ile çalışır.

Google Sheets Yapısı (Content Log):
- A sütunu: Üye isimleri (A4'ten başlar)
- A3: "Loot" header
- Her content 2 sütun kullanır: B:C, D:E, F:G...
  - B1 = "AVALON", C1 = "AVALON" (content adı her iki sütuna)
  - B2 = Tarih, C2 = Tarih  
  - B3 = Saat/Bilgi, C3 = boş
  - B4+ = Loot miktarı (katılanlara)
  - C4+ = ✅ tick (lootu aldıysa)
"""

LOOT_CHANNEL_ID = int(os.getenv("LOOT_CHANNEL_ID", "1421223928805720196"))

# Market Setup
LOOT_MARKET_SETUP_FEE = float(os.getenv("LOOT_MARKET_SETUP_FEE", "0.025"))  # 2.50%
LOOT_MARKET_TAX = float(os.getenv("LOOT_MARKET_TAX", "0.04"))  # 4.00%
LOOT_MARKET_AVG_RELIST = float(os.getenv("LOOT_MARKET_AVG_RELIST", "1.74"))  # Avg relist

# Guild Cut Setup - Brüt karın %12'si
LOOT_GUILD_CUT = float(os.getenv("LOOT_GUILD_CUT", "0.12"))  # %12 guild kesintisi

LOOT_SESSIONS: Dict[int, "LootSession"] = {}
LOOT_SESSIONS_FILE = "loot_sessions.json"
...
class LootSession:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "thread_id": self.thread_id,
            "loot_channel_msg_id": self.loot_channel_msg_id,
            "thread_msg_id": self.thread_msg_id,
            "total_loot": self.total_loot,
            "per_person_loot": self.per_person_loot,
            "participant_ids": self.participant_ids,
            "claimed_ids": list(self.claimed_ids),
            "created_at": self.created_at,
            "created_by": self.created_by,
            "sheet_loot_col": self.sheet_loot_col,
            "sheet_tick_col": self.sheet_tick_col,
            "finalized": self.finalized,
        }
    
    @classmethod
    def from_dict(cls, data: dict) -> "LootSession":
        return cls(
            content_name=data["content_name"],
            thread_id=data["thread_id"],
            loot_channel_msg_id=data["loot_channel_msg_id"],
            thread_msg_id=data["thread_msg_id"],
            total_loot=data["total_loot"],
            per_person_loot=data["per_person_loot"],
            participant_ids=data["participant_ids"],
            claimed_ids=set(data.get("claimed_ids", [])),
            created_at=data["created_at"],
            created_by=data["created_by"],
            sheet_loot_col=data.get("sheet_loot_col", 0),
            sheet_tick_col=data.get("sheet_tick_col", 0),
            finalized=data.get("finalized", False),
        )


def _save_loot_sessions():
    try:
        data = {str(k): v.to_dict() for k, v in LOOT_SESSIONS.items()}
        with open(LOOT_SESSIONS_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        log(f"[LOOT] Session kaydetme hatası: {e}")


def _load_loot_sessions():
    global LOOT_SESSIONS
    try:
        if os.path.exists(LOOT_SESSIONS_FILE):
            with open(LOOT_SESSIONS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            LOOT_SESSIONS = {int(k): LootSession.from_dict(v) for k, v in data.items()}
            log(f"[LOOT] {len(LOOT_SESSIONS)} session yüklendi")
    except Exception as e:
        log(f"[LOOT] Session yükleme hatası: {e}")
        LOOT_SESSIONS = {}


def _register_loot_views(bot_client):
    for msg_id, session in LOOT_SESSIONS.items():
        if not session.finalized:
            view = LootClaimView(msg_id)
            bot_client.add_view(view, message_id=msg_id)
    log(f"[LOOT] {len([s for s in LOOT_SESSIONS.values() if not s.finalized])} aktif view kaydedildi")


def _format_loot(n: int) -> str:
    return f"{n:,}".replace(",", ".")


def _calculate_market_expenses(chest_estimate: int) -> int:
    rate = LOOT_MARKET_SETUP_FEE + LOOT_MARKET_TAX * LOOT_MARKET_AVG_RELIST
    return int(chest_estimate * rate)


@dataclass
class LootCalculation:
    chest_estimate: int
    silver_bag: int
    repair_cost: int
    market_expenses: int
    gross_profit: int
    guild_cut_amount: int      # %12 guild kesintisi
    net_profit: int
    participants: int
    per_person: int
    
    @classmethod
    def calculate(cls, chest_estimate: int, silver_bag: int, repair_cost: int, participants: int) -> "LootCalculation":
        # Market masrafları
        market_expenses = _calculate_market_expenses(chest_estimate)
        
        # Brüt kar
        gross_profit = chest_estimate + silver_bag - repair_cost - market_expenses
        
        # Guild kesintisi (%12)
        guild_cut_amount = int(gross_profit * LOOT_GUILD_CUT)
        
        # Net kar
        net_profit = gross_profit - guild_cut_amount
        
        # Kişi başı
        per_person = math.floor(net_profit / participants) if participants > 0 else 0
        
        return cls(
            chest_estimate=chest_estimate,
            silver_bag=silver_bag,
            repair_cost=repair_cost,
            market_expenses=market_expenses,
            gross_profit=gross_profit,
            guild_cut_amount=guild_cut_amount,
...
# =========================================================
#             GOOGLE SHEETS - LOOT FUNCTIONS
# =========================================================

async def _find_or_create_loot_columns(content_name: str, content_date: str, content_time: str = "") -> Tuple[int, int]:
    """
    Find or create content columns. Content name merged across both columns.
    Returns: (loot_col, tick_col)
    """
    def _do():
        ws = _get_content_log_worksheet()
        content_lower = content_name.strip().lower()
        ...
        # Search existing - each content spans 2 columns starting from B
        col_idx = 1  # B column (0-indexed)
        while col_idx < len(row1):
            val = (row1[col_idx] or "").strip().lower()
            if val == content_lower:
                loot_col = col_idx + 1
        ...
        # Create new - find first empty pair
        next_col = 2  # B column (1-indexed)
        col_idx = 1
        while col_idx < len(row1):
            if not (row1[col_idx] or "").strip():
                next_col = col_idx + 1
        ...
        # Merge header cells and write content name
        ws.merge_cells(f"{loot_letter}1:{tick_letter}1")
        ws.update_acell(f"{loot_letter}1", content_name)
        
        # Write date to loot column only
        ws.update_acell(f"{loot_letter}2", content_date)
        
        # Write time to loot column row 3 if provided
        if content_time:
            ws.update_acell(f"{loot_letter}3", content_time)
        
        log(f"[LOOT] Sütunlar oluşturuldu: {content_name} @ {loot_letter}:{tick_letter}")
        return (loot_col, tick_col)
    
    return await run_io(_do)


async def _write_loot_to_sheet(loot_col: int, participant_data: List[Tuple[str, int]]) -> int:
    """Write loot amounts for participants. Returns count updated."""
    def _do():
        ws = _get_content_log_worksheet()
        col_letter = _col_num_to_letter(loot_col)
        col_a = ws.col_values(1)
        
        updates = []
        for display_name, loot_amount in participant_data:
            name_lower = display_name.strip().lower()
            for row_idx, cell_val in enumerate(col_a):
                if row_idx < 3:
                    continue
                cell_lower = (cell_val or "").strip().lower()
                if name_lower in cell_lower or cell_lower in name_lower:
                    updates.append({"range": f"{col_letter}{row_idx + 1}", "values": [[loot_amount]]})
                    break
        
        if updates:
            ws.batch_update(updates)
        log(f"[LOOT] {len(updates)} satıra loot yazıldı @ {col_letter}")
        return len(updates)
    
    return await run_io(_do)


async def _write_tick_to_sheet(tick_col: int, display_name: str):
    """Write ✅ tick when user claims loot."""
    def _do():
        ws = _get_content_log_worksheet()
        col_letter = _col_num_to_letter(tick_col)
        col_a = ws.col_values(1)
        ...
        for row_idx, cell_val in enumerate(col_a):
            ...
            cell_lower = (cell_val or "").strip().lower()
            if name_lower in cell_lower or cell_lower in name_lower:
                ws.update_acell(f"{col_letter}{row_idx + 1}", "✅")
                log(f"[LOOT] Tick: {display_name} @ {col_letter}{row_idx + 1}")
                return True
        return False
...
def build_loot_embed(session: LootSession, guild: discord.Guild, finalized: bool = False) -> discord.Embed:
    now_str = datetime.now(TR_TZ).strftime("%d.%m.%Y %H:%M")
    
    if finalized:
        e = discord.Embed(title=f"✅ {session.content_name} Loot Dağıtıldı!", color=0x2ECC71)
        e.add_field(name="💰 Özet", value=f"**Net Kar:** {_format_loot(session.total_loot)}\n**Kişi Başı:** {_format_loot(session.per_person_loot)}", inline=False)
        e.set_footer(text=f"Tüm lootlar dağıtıldı • İyi oyunlar! • {now_str}")
        return e
    
    e = discord.Embed(title=f"💰 {session.content_name} Loot Dağıtımı", color=0x3498DB)
    e.add_field(name="📊 Bilgi", value=f"**Net Kar:** {_format_loot(session.total_loot)}\n**Kişi Başı:** {_format_loot(session.per_person_loot)}", inline=True)
    e.add_field(name="👥 Katılımcı", value=f"{len(session.participant_ids)} kişi", inline=True)
    
    lines = []
    for uid in session.participant_ids:
        member = guild.get_member(uid)
        name = member.mention if member else f"<@{uid}>"
        lines.append(f"~~{name}~~ ✅" if uid in session.claimed_ids else name)
    
    participant_text = "\n".join(lines) if lines else "—"
    if len(participant_text) > 1024:
        participant_text = participant_text[:1000] + f"\n... +{len(session.participant_ids)} kişi"
    
    e.add_field(name="📋 Alacaklar", value=participant_text, inline=False)
    e.set_footer(text=f"İlerleme: {len(session.claimed_ids)}/{len(session.participant_ids)} kişi aldı • {now_str}")
    return e


def build_calculation_embed(calc: LootCalculation, content_name: str) -> discord.Embed:
    e = discord.Embed(title=f"📊 {content_name} - Loot Hesaplama", color=0x3498DB)
    
    e.add_field(name="📥 Girdiler", value=f"**Sandık:** {_format_loot(calc.chest_estimate)}\n**Silver Bag:** {_format_loot(calc.silver_bag)}\n**Repair:** {_format_loot(calc.repair_cost)}\n**Katılımcı:** {calc.participants}", inline=True)
    
    market_rate = LOOT_MARKET_SETUP_FEE + LOOT_MARKET_TAX * LOOT_MARKET_AVG_RELIST
    e.add_field(name="⚙️ Oranlar", value=f"**Market:** %{market_rate * 100:.2f}\n**Guild:** %{LOOT_GUILD_CUT * 100:.0f}", inline=True)
    
    calc_text = f"```\nSandık:        {calc.chest_estimate:>12,}\n+ Silver Bag:  {calc.silver_bag:>12,}\n- Repair:      {calc.repair_cost:>12,}\n- Market:      {calc.market_expenses:>12,}\n─────────────────────────\n= Brüt Kar:    {calc.gross_profit:>12,}\n- Guild (%12): {calc.guild_cut_amount:>12,}\n─────────────────────────\n= Net Kar:     {calc.net_profit:>12,}\n```".replace(",", ".")
    e.add_field(name="📋 Hesaplama", value=calc_text, inline=False)
    e.add_field(name="💰 Sonuç", value=f"**Kişi Başı:** {_format_loot(calc.per_person)}", inline=False)
    return e
...
class LootClaimView(discord.ui.View):
    def __init__(self, message_id: int):
        super().__init__(timeout=None)
        self.message_id = message_id
    
    @discord.ui.button(label="Loot'umu Aldım", style=discord.ButtonStyle.success, emoji="✅", custom_id="loot_claim_button")
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        session = LOOT_SESSIONS.get(self.message_id)
        
        if not session:
            return await interaction.response.send_message("❌ Bu loot session bulunamadı.", ephemeral=True)
        if session.finalized:
            return await interaction.response.send_message("❌ Bu loot dağıtımı zaten tamamlandı.", ephemeral=True)
        
        user_id = interaction.user.id
        if user_id not in session.participant_ids:
            return await interaction.response.send_message("❌ Bu content'e katılmadın.", ephemeral=True)
        if user_id in session.claimed_ids:
            return await interaction.response.send_message("❌ Zaten aldığını işaretledin.", ephemeral=True)
        
        session.claimed_ids.add(user_id)
        _save_loot_sessions()
        
        try:
            if session.sheet_tick_col > 0:
                await _write_tick_to_sheet(session.sheet_tick_col, interaction.user.display_name)
        except Exception as e:
            log(f"[LOOT] Tick yazma hatası: {e}")
        
        new_embed = build_loot_embed(session, interaction.guild, finalized=False)
        await interaction.response.edit_message(embed=new_embed, view=self)
        
        if len(session.claimed_ids) >= len(session.participant_ids):
            await _finalize_loot_session(session, interaction.guild)


async def _finalize_loot_session(session: LootSession, guild: discord.Guild):
    ...
    try:
        ...
        if loot_channel:
            msg = await loot_channel.fetch_message(session.loot_channel_msg_id)
            final_embed = build_loot_embed(session, guild, finalized=True)
            await msg.edit(embed=final_embed, view=None)
    except Exception as e:
        log(f"[LOOT] Finalize hatası: {e}")
    
    log(f"[LOOT] Session finalize: {session.content_name}")
...
class LootModal(discord.ui.Modal):
    def __init__(self, thread_id: int, content_name: str, participant_ids: List[int], participant_names: List[str]):
        super().__init__(title="Loot Dağıtımı")
        self.thread_id = thread_id
        self.content_name = content_name
        self.participant_ids = participant_ids
        self.participant_names = participant_names
        
        self.chest_input = discord.ui.TextInput(label="Sandık Tahmini (Chest Estimate)", placeholder="Örn: 9663900", required=True, max_length=20)
        self.silver_bag_input = discord.ui.TextInput(label="Silver Bag (Direkt Silver)", placeholder="Örn: 2080000 (0 için boş bırak)", required=False, max_length=20, default="0")
        self.repair_input = discord.ui.TextInput(label="Repair Ücreti", placeholder="Örn: 692000", required=True, max_length=20)
        
        self.add_item(self.chest_input)
        self.add_item(self.silver_bag_input)
        self.add_item(self.repair_input)
    
    def _parse_number(self, value: str) -> int:
        if not value or not value.strip():
            return 0
        return int(value.replace("_", "").replace(".", "").replace(",", "").replace(" ", "").strip())
    
    async def on_submit(self, interaction: discord.Interaction):
        await safe_defer(interaction, ephemeral=True)
        
        try:
            chest_estimate = self._parse_number(self.chest_input.value)
            silver_bag = self._parse_number(self.silver_bag_input.value or "0")
            repair_cost = self._parse_number(self.repair_input.value)
            
            if chest_estimate <= 0:
                return await safe_send(interaction, "❌ Sandık tahmini 0'dan büyük olmalı.", ephemeral=True)
            
            participants = len(self.participant_ids)
            calc = LootCalculation.calculate(chest_estimate, silver_bag, repair_cost, participants)
            
            if calc.net_profit <= 0:
                return await safe_send(interaction, f"❌ Net kar 0 veya negatif! ({_format_loot(calc.net_profit)})", ephemeral=True)
            if calc.per_person <= 0:
                return await safe_send(interaction, "❌ Kişi başı loot 0 veya negatif!", ephemeral=True)
            
            for session in LOOT_SESSIONS.values():
                if session.thread_id == self.thread_id and not session.finalized:
                    return await safe_send(interaction, "❌ Bu thread için zaten aktif bir loot dağıtımı var!", ephemeral=True)
            ...
            if not loot_channel:
                return await safe_send(interaction, f"❌ Loot kanalı bulunamadı! (ID: {LOOT_CHANNEL_ID})", ephemeral=True)
            
            sheet_loot_col, sheet_tick_col = 0, 0
            try:
                if GOOGLE_CREDS_JSON and ACTIVITY_SHEET_ID:
                    date_str = datetime.now(TR_TZ).strftime("%Y-%m-%d")
                    time_str = datetime.now(TR_TZ).strftime("%H:%M")
                    sheet_loot_col, sheet_tick_col = await _find_or_create_loot_columns(self.content_name, date_str, time_str)
                    participant_data = [(name, calc.per_person) for name in self.participant_names]
                    await _write_loot_to_sheet(sheet_loot_col, participant_data)
            except Exception as e:
                log(f"[LOOT] Sheet hatası: {e}")
            
            session = LootSession(
                content_name=self.content_name,
                thread_id=self.thread_id,
                loot_channel_msg_id=0,
                thread_msg_id=0,
                total_loot=calc.net_profit,
                per_person_loot=calc.per_person,
                participant_ids=self.participant_ids,
                claimed_ids=set(),
                created_at=datetime.now(UTC_TZ).isoformat(),
                created_by=interaction.user.id,
                sheet_loot_col=sheet_loot_col,
                sheet_tick_col=sheet_tick_col,
                finalized=False,
            )
            
            announcement_embed = build_loot_embed(session, guild, finalized=False)
            
            try:
                loot_msg = await loot_channel.send(embed=announcement_embed, view=LootClaimView(0))
                session.loot_channel_msg_id = loot_msg.id
                proper_view = LootClaimView(loot_msg.id)
                await loot_msg.edit(view=proper_view)
                bot.add_view(proper_view, message_id=loot_msg.id)
            except Exception as e:
                return await safe_send(interaction, f"❌ Loot kanalına mesaj gönderilemedi: {e}", ephemeral=True)
            
            try:
                if thread and isinstance(thread, discord.Thread):
                    calc_embed = build_calculation_embed(calc, self.content_name)
                    thread_msg = await thread.send(embed=calc_embed)
                    session.thread_msg_id = thread_msg.id
            except Exception as e:
                log(f"[LOOT] Thread mesajı hatası: {e}")
            
            LOOT_SESSIONS[session.loot_channel_msg_id] = session
            _save_loot_sessions()
            
            await safe_send(interaction, f"✅ **Loot dağıtımı başlatıldı!**\n\n**Content:** {self.content_name}\n**Net Kar:** {_format_loot(calc.net_profit)}\n**Katılımcı:** {participants}\n**Kişi başı:** {_format_loot(calc.per_person)}\n\nDuyuru <#{LOOT_CHANNEL_ID}> kanalına gönderildi.", ephemeral=True)
            
        except ValueError as e:
            await safe_send(interaction, f"❌ Geçersiz sayı formatı: {e}", ephemeral=True)
        except Exception as e:
            log(f"[LOOT] Modal hatası: {e}")
            await safe_send(interaction, f"❌ Hata: {e}", ephemeral=True)
...
@bot.tree.command(name="loot", description="Loot Dağıtım (sadece thread)", guild=discord.Object(id=GUILD_ID))
async def loot_cmd(interaction: discord.Interaction):
    channel = interaction.channel
    if not isinstance(channel, discord.Thread):
        return await safe_send(interaction, "❌ Bu komut sadece bir content thread'i içinde kullanılabilir!", ephemeral=True)
    
    if not _is_staff_member(interaction):
        return await safe_send(interaction, "❌ Bu komutu kullanma yetkin yok.", ephemeral=True)
    
    thread_id = channel.id
    main_msg_id = SHEET_THREAD_TO_MAIN.get(thread_id)
    
    content_name = ""
    participant_ids: List[int] = []
    participant_names: List[str] = []
    
    if main_msg_id and main_msg_id in SHEET_EVENTS:
        sheet_state = SHEET_EVENTS[main_msg_id]
        content_name = (sheet_state.title or "").strip() or "CONTENT"
        for uid in sheet_state.user_slot.keys():
    ...
    if not participant_ids:
        for msg_id, evt_state in EVENTS.items():
            if evt_state.thread_id == thread_id:
                content_name = (evt_state.template.thread_name or evt_state.template.key or "").strip() or "CONTENT"
                seen = set()
                for role, _cap in evt_state.template.roles:
                    for uid in evt_state.roster.get(role, []):
                        if uid not in seen:
                            participant_ids.append(uid)
                            seen.add(uid)
                            member = interaction.guild.get_member(uid)
    ...
    if not content_name:
        content_name = channel.name.upper()
    
    if not participant_ids:
        return await safe_send(interaction, "❌ Bu thread'de katılımcı bulunamadı. Content kapatılmamış veya kimse katılmamış olabilir.", ephemeral=True)
    
    for session in LOOT_SESSIONS.values():
        if session.thread_id == thread_id and not session.finalized:
            return await safe_send(interaction, f"❌ Bu thread için zaten aktif bir loot dağıtımı var!", ephemeral=True)
    
    try:
        modal = LootModal(thread_id, content_name, participant_ids, participant_names)
        await interaction.response.send_modal(modal)
    except Exception as e:
        log(f"[LOOT] Modal açma hatası: {e}")
        await safe_send(interaction, f"❌ Modal açılamadı: {e}", ephemeral=True)
