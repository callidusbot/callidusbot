...
class ContentSelect(discord.ui.Select):
    def __init__(self):
        ...
        # dynamic sheets - sadece "content" tipindekiler (25 option limit)
        try:
            dyn_items = sorted(
...
class MassSelect(discord.ui.Select):
    """Mass content için select menu - sadece 'mass' tipindeki sheet'ler"""
    def __init__(self):
        opts: List[discord.SelectOption] = []
        ...
        if not dyn_items:
            # Hiç mass sheet yoksa bilgi ver
            opts.append(discord.SelectOption(label="Mass sheet yok", value="none", emoji="❌"))
        else:
            for k, cfg in dyn_items[:24]:  # Max 25, 1 boşluk bırak
                name = (cfg.get("name") or k).strip() or k
                emoji = (cfg.get("emoji") or "⚔️").strip() or "⚔️"
                opts.append(discord.SelectOption(label=name[:100], value=f"mass_dyn:{k}", emoji=emoji))

        super().__init__(placeholder="👇 Mass seç", min_values=1, max_values=1, options=opts)

    async def callback(self, interaction: discord.Interaction):
        key = (self.values[0] or "").strip()

        if key == "none":
            return await safe_send(interaction, "❌ Henüz mass sheet eklenmemiş.\n`/sheetekle` ile tip: `mass` seçerek ekleyebilirsin.", ephemeral=True)
...
#            self._kb_task = asyncio.create_task(self._killbot_loop())
        # Web Panel queue listener (file-based, no HTTP server)
        if getattr(self, '_queue_task', None) is None:
            self._queue_task = self.loop.create_task(self.command_queue_loop())
            log('[QUEUE] File-based command listener started')

    # ---------------------------------------------------------
    # WEB PANEL INTEGRATION (FILE-BASED QUEUE)
    # ---------------------------------------------------------
    def _panel_paths(self):
        data_dir = os.getenv("BOT_DATA_PATH", "data")
        commands_dir = os.getenv("BOT_COMMANDS_PATH", "bot_commands")
        return data_dir, commands_dir

    def _panel_read_json(self, filename: str, default=None):
        data_dir, _ = self._panel_paths()
        ...
        try:
            ...
            log(f"[PANEL] JSON read error {filename}: {e}")
            return default

    def _panel_write_json_atomic(self, filename: str, payload):
        data_dir, _ = self._panel_paths()
        ...
        try:
            ...
            log(f"[PANEL] JSON write error {filename}: {e}")
            try:
    ...
    def _sync_active_events_for_panel(self):
        out = []
        try:
            for msg_id, st in SHEET_EVENTS.items():
                participants = []
                try:
                    for role_list in st.roster.values():
                        for uid in role_list:
                ...
                out.append({
                    "event_id": f"msg_{msg_id}",
                    "message_id": msg_id,
                    "template_key": getattr(st, "template_key", None),
                    "started_by_name": getattr(st, "started_by_name", "Unknown"),
                    "started_at": getattr(st, "started_at", 0),
                    "thread_id": getattr(st, "thread_id", None),
                    "participants": participants,
                    "status": "closed" if getattr(st, "closed", False) else "active"
                })
        except Exception as e:
            log(f"[PANEL] sync_active_events failed: {e}")

        self._panel_write_json_atomic("active_events.json", out)

    async def command_queue_loop(self):
        _, commands_dir = self._panel_paths()
        os.makedirs(commands_dir, exist_ok=True)
        queue_path = os.path.join(commands_dir, "queue.jsonl")
        self._sync_active_events_for_panel()

        while not self.is_closed():
            try:
                if os.path.exists(queue_path) and os.path.getsize(queue_path) > 0:
                    lock_path = queue_path + ".processing"
                    try:
                        os.replace(queue_path, lock_path)
                    except Exception:
                        lock_path = None
                        ...
                        with open(lock_path, "r", encoding="utf-8") as f:
                            for line in f:
                                ...
                                try:
                                    ...
                                    continue
                                try:
                                    await self._handle_queue_command(cmd)
                                except Exception as e:
                                    log(f"[QUEUE] Command error: {e}")
                        try:
                            os.remove(lock_path)
                        except Exception:
                            pass
    ...
    async def _handle_queue_command(self, cmd: dict):
        action = cmd.get("action")
        if not action:
            return
        log(f"[QUEUE] Handling: {action}")

        if action == "close_content":
            event_id = cmd.get("event_id")
            thread_id = cmd.get("thread_id")
            start_loot = bool(cmd.get("start_loot", False))
            closed_by = cmd.get("requested_by", "Panel Admin")
            message_id = None

            if isinstance(event_id, str) and event_id.startswith("msg_"):
                try:
                    message_id = int(event_id.split("_", 1)[1])
                except Exception:
                    message_id = None
            if message_id is None and isinstance(event_id, (int, float, str)):
                try:
            ...
            if message_id is None and thread_id:
                try:
                    message_id = SHEET_THREAD_TO_MAIN.get(int(thread_id))
                except Exception:
                    message_id = None
            ...
            await self._internal_close_content(
                message_id=int(message_id),
                start_loot=start_loot,
                closed_by_name=str(closed_by)
            )

    async def _internal_close_content(self, message_id: int, start_loot: bool, closed_by_name: str):
        st = SHEET_EVENTS.get(message_id)
        if not st:
            return False

        guild = self.get_guild(GUILD_ID)
        thread = guild.get_thread(st.thread_id) if guild else self.get_channel(st.thread_id)
        if not thread:
            thread = self.get_channel(st.thread_id)
        if not thread:
            return False
        ...
        await thread.send(
            f"🔒 **Content Web Panel Tarafından Kapatıldı**\n"
            f"👤 Kapatan: {closed_by_name}\n"
            f"📊 Katılımcı: {len(participants)}"
        )

        try:
            if st.thread_id in SHEET_THREAD_TO_MAIN:
                del SHEET_THREAD_TO_MAIN[st.thread_id]
        except Exception:
            pass
        try:
            del SHEET_EVENTS[message_id]
        except Exception:
            pass
        try:
            _save_sheet_events()
        ...
        if start_loot:
            await thread.send("⚔️ **Loot Dağıtımı Başlatıldı!**\nLütfen loot görüntüsünü yükleyin.")
...
def _sync_active_events_for_panel(self):
    """Exports current active sheet events to data/active_events.json for the web panel."""
    out = []
    try:
        for msg_id, st in SHEET_EVENTS.items():
            # Participants may be nested by role; flatten + unique
            participants = []
            try:
                for role_list in st.roster.values():
                    for uid in role_list:
...
async def command_queue_loop(self):
    """Watches bot_commands/queue.jsonl and executes commands written by the web panel."""
    _, commands_dir = self._panel_paths()
    os.makedirs(commands_dir, exist_ok=True)
    queue_path = os.path.join(commands_dir, "queue.jsonl")

    # initial export so panel can show current state even before first event
    self._sync_active_events_for_panel()
    ...
    while not self.is_closed():
        try:
            if os.path.exists(queue_path) and os.path.getsize(queue_path) > 0:
                lock_path = queue_path + ".processing"
                try:
                    os.replace(queue_path, lock_path)  # atomic lock
                except Exception:
                    lock_path = None
                    ...
                    with open(lock_path, "r", encoding="utf-8") as f:
                        for line in f:
                            ...
                            try:
                                ...
                                log("[QUEUE] Bad JSON line, skipping")
                                continue
                            try:
                                await self._handle_queue_command(cmd)
                            except Exception as e:
...
async def _handle_queue_command(self, cmd: dict):
    ...
    if action == "close_content":
        ...
        # If panel sends numeric message id directly
        if message_id is None and isinstance(event_id, (int, float, str)):
            try:
                if str(event_id).isdigit():
                    message_id = int(event_id)
        ...
        # If we only have thread_id, map to main message id
        if message_id is None and thread_id:
            try:
                message_id = SHEET_THREAD_TO_MAIN.get(int(thread_id))
            except Exception:
        ...
        # Fallback: lookup from active_events.json
        if message_id is None and event_id:
            data = self._panel_read_json("active_events.json", default=[])
            if isinstance(data, list):
                for row in data:
        ...
        if message_id is None:
            log(f"[QUEUE] close_content: cannot resolve message_id for event_id={event_id} thread_id={thread_id}")
            return
        ...
        # You can wire this to your existing points reset logic later.
        log("[QUEUE] reset_points received (no-op for now).")

async def _internal_close_content(self, message_id: int, start_loot: bool, closed_by_name: str):
    """Headless close: works without Interaction (web panel / automations)."""
    st = SHEET_EVENTS.get(message_id)
    if not st:
        log(f"[QUEUE] _internal_close_content: sheet event not found message_id={message_id}")
        return False

    # Resolve guild + thread
    guild = self.get_guild(GUILD_ID)
    thread = None
    try:
        if guild:
            thread = guild.get_thread(st.thread_id) or self.get_channel(st.thread_id)
        else:
            thread = self.get_channel(st.thread_id)
    except Exception:
        thread = self.get_channel(st.thread_id)
    ...
    if not thread:
        log(f"[QUEUE] _internal_close_content: thread not found thread_id={st.thread_id}")
        return False

    # Participants count (best-effort)
    participants = []
    try:
        for role_list in st.roster.values():
            for uid in role_list:
    ...
    await thread.send(
        f"🔒 **Content Web Panel Tarafından Kapatıldı**\n"
        f"👤 Kapatan: {closed_by_name}\n"
        f"📊 Katılımcı Sayısı: {len(participants)}\n"
        f"💰 Loot Durumu: {'Dağıtılacak' if start_loot else 'Yok'}"
    )
    ...
    if start_loot:
        await thread.send(
            "⚔️ **Loot Dağıtımı Başlatıldı!**\n"
            "Lütfen loot görüntüsünü buraya yükleyin ve ilgili loot komutunu kullanın."
        )
...
async def _search_albion_player(bot_client, name: str):
    ...
    This function intentionally does NOT rely on any optional killboard helpers.
    It uses a small safe aiohttp request so /bagla keeps working even if killboard
    modules are removed/disabled.
    """
    try:
        import aiohttp
        url = f"{AO_API_BASE}/search?q={name}"
        timeout = aiohttp.ClientTimeout(total=15)
        headers = {"User-Agent": "CallidusBot/1.0", "Accept": "application/json,text/plain,*/*"}
            ...
            async with session.get(url) as r:
                if r.status != 200:
                    try:
                        ...
                        body = ""
                    log(f"albion search HTTP {r.status}: {body[:200]}")
                    return None
                try:
